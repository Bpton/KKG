...
function addgrid() {
    ...
    if ($('#grdProduct').length) {
        $("#grdProduct tbody tr").each(function () {
            var narr = $(this).find('td:eq(3)').html();
            if (NARRATION == narr) {
                isexists = 'y';
                return false;
            }
    ...
    $.ajax({
        ...
        url: "/claims/addGiftGrid",
        data: {
            FROMDT: FROMDT, TODT: TODT, PRODUCTID: PRODUCTID, PRODUCTNAME: PRODUCTNAME, Amount: Amount, Qty:Qty, NARRATIONID: NARRATIONID, NARRATION: NARRATION
        },
        ...
        success: function (response) {
            ...
            tableNew = tableNew + '<thead ><tr><th style="display:none"></th><th >Sl.No.</th><th> CLAIM DETAILS</th><th> PRODUCT DETAILS</th><th> FROM DATE</th><th>TO DATE</th><th>CLAIM QTY</th><th>DELETE</th></tr></thead><tbody>';
            var totQty = 0;
            var counter = 0;
            ...
            $.each(response, function () {
                counter = counter + 1;
                tableNew = tableNew + "<tr><td style='display:none'>" + this["GUID"] + "</td><td>" + counter + "</td><td>" + this["NARRATION"] + "</td><td>" + this["PRODUCTNAME"] + "</td>  <td>" + this["FROM_DATE"] + "</td><td>" + this["TO_DATE"] + "</td><td>" + this["QTY"] + "</td>    <td><input type='image' class='grvdtldel' id='btnDtldel' src='../Images/ico_delete_16.png' title='Delete' /></td>";

                totQty = totQty + parseFloat(this["QTY"])

            })
            document.getElementById("grdProduct").innerHTML = tableNew + '</tbody></table>';
            $("#txtClaimQty").val(totQty);
            $("#txtQty").val('0');
            $("#txtStartDate").val('');
            $("#txtEndDate").val('');
            $("#txtnarration").val('0');
...
function SaveGiftClaim() {
    ...
    if ($('#txtClaimQty').val() == 0) {
        toastr.warning('Total Qty Cannot be zero');
        return;
    }
...
function ReleaseSession() {
    $.ajax({
        type: "POST",
        url: "/claims/RemoveGiftClaimeSession",
        data: '{}',
        dataType: "json",
        success: function (response) {
...
function delgrid(guid) {
    $.ajax({
        ...
        success: function (response) {
            var tableNew = '<table id="grdProduct" class="table table-striped table-bordered dt-responsive nowrap">';
            tableNew = tableNew + '<thead ><tr><th style="display:none"></th><th >Sl.No.</th><th> CLAIM DETAILS</th><th> FROM DATE</th><th>TO DATE</th><th>CLAIM QTY</th><th>DELETE</th></tr></thead><tbody>';
            var totalamt = 0;
            var counter = 0;
            ...
            $.each(response, function () {
                counter = counter + 1;
                tableNew = tableNew + "<tr><td style='display:none'>" + this["GUID"] + "</td><td>" + counter + "</td><td>" + this["NARRATION"] + "</td>  <td>" + this["FROM_DATE"] + "</td><td>" + this["TO_DATE"] + "</td><td>" + this["QTY"] + "</td>    <td><input type='image' class='grvdtldel' id='btnDtldel' src='../Images/ico_delete_16.png' title='Delete' /></td>";

                totalamt = totalamt + parseFloat(this["QTY"])

            })
            document.getElementById("grdProduct").innerHTML = tableNew + '</tbody></table>';
            $("#txtClaimQty").val(totalamt);
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function () {
                ...
                totalQty = totalQty + parseFloat(this["QTY"])

            })
            document.getElementById("grdProduct").innerHTML = tableNew + '</tbody></table>';
            $("#txtClaimQty").val(totalQty);
        }
    })
}
...
function loadProduct() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $("#ddlProduct").chosen('destroy');
            $("#ddlProduct").chosen({ width: '150px' });
        },
