...
$(document).ready(function () {
    ...
    $('#ddltodepot').change(function () {
        ...
        if (todepoid == '0') {
            if (waybillapp == '0') {
                ...
                if (waybillapp != '0') {
                    $("#ddlwaybill").empty().append('<option selected="selected" value="0">Select Ewatbill Key </option>');
                }
        }
        else
            if (todepoid != '0') {
    ...
    $("#btnADDGrid").click(function (e) {
        ...
        if (transferqty == 0) {
            ...
            if (hdnrate == '0.00') {
                toastr.warning('<b><font color=black>Rate cannot be 0..!</font></b>');
                return false;
            }
    ...
    $("body").on("click", "#productDetailsGrid .gvTempDelete", function () {
        ...
        var grdmfgDate = row.find('td:eq(14)').html();
        var grdexprDate = row.find('td:eq(15)').html();
        
        //logic for fifo
        $('#productDetailsGrid tbody tr').each(function () {
            pid = $(this).find('td:eq(1)').html().trim();
            mfgDate = $(this).find('td:eq(14)').html().trim();
            exprDate = $(this).find('td:eq(15)').html().trim();
            if (grdproductid == pid) {
               
                if (grdmfgDate != mfgDate && grdexprDate != exprDate) {
                    if (new Date(grdmfgDate.replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$2/$1/$3")) < new Date(mfgDate.replace(/(\d{2})\/(\d{2})\/(\d{4})/, "$2/$1/$3"))) {
                        fifoflag = 1;
                        return false;
                    }
                    else {
    ...
    $("body").on("click", "#dispatchGrid .gvPrint", function () {
        ...
        //var url = "http://localhost:2319/view/frmPrintPopUpv2.aspx?Stnid=" + dispatchid + "&BSID=1&pid=1&MenuId=" + MENUID;
            ...
            if (listDetails.length > 0) {
                ...
                if ($("#hdntaxcount").val().trim() == '1') {
                    ...
                    $.each(listDetails, function (index, record) {
                        ...
                        tr.append("<td style='text-align: right'>" + TaxPercentage + "</td>");//19
                    ...
                    HeaderCount = $('#productDetailsGrid thead th').length;
                    if (HeaderCount == 0) {
                        tr.append("<thead><tr><th>Sl.No.</th><th style='display: none'>Product ID</th><th>HSN Code</th><th>Product</th><th>Batch</th><th style='display: none'>PackSize ID</th><th>Pack Size</th><th>Qty</th><th>MRP</th><th>Rate</th><th>Amount</th><th style='display: none'>Assesmet(%)</th><th style='display: none'>Assesment Amt.</th><th style='display: none'>Net Wght.</th><th>Gross Wght.</th><th>Mfg.Date</th><th>Exp.Date</th><th style='display: none'>Tag</th><th style='display: none'>Before Edited Qty</th><th style='display: none'>IGSt%</th><th style='display: none'>IGSt Amt.</th><th>Net Amt.</th><th>Delete</th></tr></thead><tbody>");
                    }
                    $.each(listDetails, function (index, record) {
                        ...
                        if ($.trim(this['GROSSWEIGHT']).length==0) {
                            tr.append("<td >0 </td>");//14
                        }
                        else {
                        ...
                        tr.append("<td style='display: none'>" + TaxPercentage + "</td>");//18
                        tr.append("<td style='display: none'>" + TaxAmount + "</td>");//18
                        NetAmount = parseFloat(this['AMOUNT'].toString().trim());
                        tr.append("<td style='text-align: right'>" + NetAmount + "</td>");//10 + "</td>");//19
                        tr.append("<td style='text-align: center'><input type='image' class='gvTempDelete'  id='btndelete' <img src='../Images/ico_delete_16.png'/></input></td>");
                        $("#productDetailsGrid").append(tr);
                        tr.append("</tbody>");
...
function bindstocktransfer() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/Bindstocktransfer",
...
function bindstocktransfer1() {
    ...
    var srl = 0;
    var frmdate = $('#txtfrmdate').val().trim();
    var todate = $('#txttodate').val().trim()
    var tr;
    document.getElementById("dispatchGrid").deleteTHead();
    tr = $('#dispatchGrid');
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/Bindstocktransfer",
        data: { FromDate: frmdate, ToDate: todate, CheckerFlag: CHECKER, depotID: DEPOTID, finyr: FINYEAR },
       // data: '{ }',
        async: false,
        dataType: "json",
        ...
        success: function (response) {
          ...
          // RowCountDispatchList();
            ...
            //    "ordering": false
            //});
            //document.getElementById("DivHeaderRow").innerHTML = tr + '</table>';
...
function RowCountDispatchList() {
    var table = document.getElementById("dispatchGrid");
    var rowCount = document.getElementById("dispatchGrid").rows.length - 1;
    if (rowCount > 0) {
        for (var i = 1; i <= rowCount; i++) {
            table.rows[i].cells[0].innerHTML = i.toString();
        }
...
function Savestktransfer() {
    ...
    if (Taxreturnflag == true) {
        ...
        Depostockmodel.INVOICETYPE = parseInt($('#hdntaxcount').val().trim());
        ...
        Depostockmodel.TAXCOUNT = taxcount;
        ...
        freelist[0] = gtfreedetails;
        Depostockmodel.Freestock = freelist;

        $.ajax({

            url: "/TranDepotStock/Depotstocktransave",
...
function addProductInDetailsGrid() {
    ...
    var totalmrp = 0;
    var MRP = $('#txtmrp').val();
    var taxname = $('#hdntaxname').val();
    
    var AssesmentPercentage = $("#hdn_ASSESMENTPERCENTAGE").val();
    //rate = $("#hdnrate").val();
    rate = (parseFloat($("#txtmrp").val()) * parseFloat($("#hdn_ASSESMENTPERCENTAGE").val()))/100;
    ...
    var srl = 0;
   // alert($("#hdn_ASSESMENTPERCENTAGE").val());
    //alert($("#hdnrate").val());
    
    srl = srl + 1; 
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/GetCalculateAmtInPcs",
        data: { Productid: productid.trim(), PacksizeID: packsizeid, Qty: transferqty, Rate: rate, date: date, _qty: stkqty, taxname: taxname },
        async: false,
        dataType: "json",
        success: function (response) {
          //  debugger;
            ...
            if ($("#hdntaxcount").val().trim() == '1') {
                ...
                tr.append("<td style='text-align: right'>" + $("#hdnrate").val() + "</td>");//9
              
                tr.append("<td style='text-align: right'>" + Amount + "</td>");//10
                tr.append("<td style='display: none'>" + totalmrp.toFixed(2) + "</td>");//11
                ...
                tr.append("<td style='text-align: center'>" + $('#hdn_mfgdate').val() + "</td>");//16
                tr.append("<td style='text-align: center'>" + $('#hdn_exprdate').val() + "</td>");//17
                tr.append("<td style='display: none'>" + Tag + "</td>");//18
                tr.append("<td style='display: none'>" + BeforeEditedQty + "</td>");//19
                tr.append("<td style='text-align: right'>" + TaxPercentage.toFixed(2) + "</td>");//20
                tr.append("<td style='text-align: right'>" + TaxAmount.toFixed(2) + "</td>");//21
                ...
                if (HeaderCount == 0) {
                   // tr.append("<thead><tr><th>Sl.No.</th><th style='display: none'>Product ID</th><th>HSN Code</th><th>Product</th><th>Batch</th><th style='display: none'>PackSize ID</th><th>Pack Size</th><th>Qty</th><th>MRP</th><th>Rate</th><th>Amount</th><th style='display: none'>Total Mrp</th><th style='display: none'>Assesmet(%)</th><th style='display: none'>Assesment Amt.</th><th style='display: none'>Net Wght.</th><th>Gross Wght.</th><th>Mfg.Date</th><th>Exp.Date</th><th style='display: none'>Tag</th><th style='display: none'>Before Edited Qty</th><th>Net Amt.</th><th>Delete</th></tr></thead><tbody>");
                    tr.append("<thead><tr><th>Sl.No.</th><th style='display: none'>Product ID</th><th>HSN Code</th><th>Product</th><th>Batch</th><th style='display: none'>PackSize ID</th><th>Pack Size</th><th>Qty</th><th>MRP</th><th>Rate</th><th>Amount</th><th style='display: none'>MRP</th><th style='display: none'>Assesmet(%)</th><th style='display: none'>Assesment Amt.</th><th style='display: none'>Net Wght.</th><th>Gross Wght.</th><th>Mfg.Date</th><th>Exp.Date</th><th style='display: none'>Tag</th><th style='display: none'>Before Edited Qty</th><th style='display: none'>IGST(%)</th><th style='display: none'>IGST</th><th>Net Amt.</th><th>Delete</th></tr></thead><tbody>");
                }
                tr = $('<tr/>');
                tr.append("<td style='text-align: center'>" + srl + "</td>");//0
                tr.append("<td style='display: none'>" + productid + "</td>");//1
                ...
                tr.append("<td style='text-align: right'>" + MRP + "</td>");//8
                tr.append("<td style='text-align: right'>" + $("#hdnrate").val() + "</td>");//9
                tr.append("<td style='text-align: right'>" + Amount.toFixed(2) + "</td>");//9
                ...
                tr.append("<td style='text-align: right; display: none'>" + TaxPercentage.toFixed(2) + "</td>");//19
                tr.append("<td style='text-align: right; display: none'>" + TaxAmount.toFixed(2) + "</td>");//20
                tr.append("<td style='text-align: right'>" + NetAmount.toFixed(2) + "</td>");//21
                tr.append("<td style='text-align: center'><input type='image' class='gvTempDelete'  id='btndelete' <img src='../Images/ico_delete_16.png'/></input></td>");
                $("#productDetailsGrid").append(tr);
                tr.append("</tbody>");
    ...
    if (rowCount > 0) {
        if ($("#hdntaxcount").val() == '1') {
            $('#productDetailsGrid tbody tr').each(function () {
                ...
                totaltaxamt += parseFloat($(this).find('td:eq(21)').html().trim());
        ...
        $('#txttotal').val(totalbasicplustaxamt.toFixed(2));
        ...
        //  removed for round up
        // finalamt = Math.round(totalbasicplustaxamt);
        // changed for round up
...
function CalculateAmountedit() {
    ...
    if (rowCount > 0) {
        ...
        $('#txtTotalGross').val(totalbasicamt.toFixed(2));
        parts = totalbasicplustaxamt - Math.floor(totalbasicplustaxamt);
        decimalvalue = parts.toFixed(2);
        //change for round up
        //if (decimalvalue >= .50) {
    ...
    if (rowCount > 0) {
        $('#productDetailsGrid tbody tr').each(function () {
            ...
            $.ajax({
                ...
                data: { Productid: $(this).find('td:eq(1)').html().trim(), FromPackSizeID: $(this).find('td:eq(5)').html(), Qty: parseFloat($(this).find('td:eq(7)').html()) },
                async: false,
                dataType: "json",
                success: function (response) {
...
function addRowinTaxTable(TaxPercentage, TaxAmount, MRP, TaxFlag) {
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/FillTaxDatatable",
...
function addRowinTaxTableEdit(productid,batchno,taxid, TaxPercentage, TaxAmount, MRP, TaxFlag) {
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/FillTaxDatatableEdit",
...
function deleteRowfromTaxTable(grdproductid, grdbatchno) {
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/DeleteTaxDatatable",
        data: { Productid: grdproductid, BatchNo: grdbatchno },
        dataType: "json",
        success: function (response) {
...
function Bindmodeoftransport() {
    ...
    listItems += "<option value='BY RAI'>BY RAIL</option>";
...
function Bindpacksize() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(response, function () {
                ddlpackingsize.append($("<option></option>").val(this['psid']).html(this['psname']));
            });
            $("#ddlpackingsize").chosen({
                search_contains: true
            });
...
function BindCATEGORY() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/BindCATEGORY",
        data: '{}',
        async: false,
        dataType: "json",
...
function bindallproduct() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/LoadProduct",
...
function BindProduct_depotwise() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/BindProduct_depotwise",
        data: { depotid: sourcedepo, todepotid: todepotid},
        async: false,
        dataType: "json",
        success: function (response) {
            //alert(JSON.stringify(response));
...
function LoadCategoryWiseProduct() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/LoadCategoryWiseProduct",
        ...
        success: function (Batchdetail) {
            ...
            if ($('#productDetailsGrid').length) {
                ...
                $('#productDetailsGrid tbody tr').each(function () {
                    ...
                    BillProductBatchNo = $(this).find('td:eq(4)').html().trim();
                    if (productid.trim() == BillProductId.trim()) {

                        for (var j = 0; j < Batchdetail.length; j++) {

                            var stockBatchNo = Batchdetail[j].BATCHNO.toString().trim();

                            if (stockBatchNo == BillProductBatchNo) {
                                var index = j;
                                Batchdetail.splice(index, 1);
                            }
                        }
                    }
...
function BindSTNInsuranceNumber() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (counter == 1) {
                ddlinsuranceno.prop('selectedIndex', 1);
            }
            //$("#ddlinsuranceno").clearcontrols({
            //    search_contains: true
            //});
            //$("#ddlinsuranceno").trigger("chosen:updated");
...
function ReleaseSession() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepotStock/RemoveSession",
        data: '{}',
        dataType: "json",
        success: function (response) {
