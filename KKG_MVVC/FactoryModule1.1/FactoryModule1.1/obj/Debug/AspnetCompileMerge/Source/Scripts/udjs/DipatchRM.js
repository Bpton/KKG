...
/*
* For FactoryDispatchRMPM.cshtml Only
...
function bindProduct(sourceDepot) {
    ...
    $.ajax({
        ...
        data: { SourceDepot: sourceDepot,Type: 'RM' },
        async: false,
        dataType: "json",
        success: function (response) {
...
function bindPacksize() {
    ...
    $.ajax({
        ...
        data: { ProductID: $('#PRODUCTID').val(), Type: 'RM' },
        async: false,
        dataType: "json",
        success: function (response) {
...
function bindCategorywiseProduct() {
    ...
    $.ajax({
        ...
        data: { CategoryID: $('#CATID').val(), DepotID: $('#BRID').val(), Type: 'RM' },
        dataType: "json",
        success: function (response) {
            Product.empty().append('<option selected="selected" value="0">Select Product</option>');
            $.each(response, function () {
...
function bindAllProduct() {
    ...
    $.ajax({
        type: "POST",
        url: "/tranfac/GetProduct",
        data: { SourceDepot: $('#BRID').val(), Type: 'RM' },
        dataType: "json",
        success: function (response) {
            Product.empty().append('<option selected="selected" value="0">Select Product</option>');
            $.each(response, function () {
...
function addProductInDetailsGrid() {
    ...
    var DiscRate = 0;
    var dispatchqty = 0;
    dispatchqty = parseFloat($('#DispatchQty').val().trim());
    debugger;
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            //Add Mode
            if ($("#hdntaxcount").val().trim() == '1') {
                if ($('#PSID').val().trim() == 'B9F29D12-DE94-40F1-A668-C79BF1BF4425' || $('#PSID').val().trim() == '1970C78A-D062-4FE9-85C2-3E12490463AF') {
                    Amount = parseFloat(qty * DiscRate);
                }
                else {
                    Amount = parseFloat(dispatchqty * DiscRate);
                }
                ...
                tr.append("<td style='text-align: right'>" + Amount.toFixed(2) + "</td>");//10
...
function addRowinTaxTable(TaxPercentage, TaxAmount, MRP, TaxFlag) {
    $.ajax({
        type: "POST",
        url: "/tranfac/FillTaxDatatableRM",
...
function deleteRowfromTaxTable(grdproductid, grdbatchno) {
    $.ajax({
        type: "POST",
        url: "/tranfac/DeleteTaxDatatableRM",
...
function ClearControls() {
    ...
    $("#DeliveryDate").val('');
    debugger;
    //backdatestatus = bindbackdateflag('4191');
    ...
    if (Taxreturnflag == true) {
        ...
        dispatchsave.DispatchDetailsFG = dispatchlist,
            ...
            $.ajax({
                ...
                url: "/tranfac/rmDispatchsavedata",
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                tr.append("<td style='display: none'>" + response[i].TOTALCASE + "</td>");
