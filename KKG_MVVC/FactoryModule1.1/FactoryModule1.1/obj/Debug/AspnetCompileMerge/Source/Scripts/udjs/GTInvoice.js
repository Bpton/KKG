...
$(document).ready(function () {
    ...
    $('#GroupID').change(function (e) {
        ...
        if ($('#GroupID').val != '0') {
            ...
            $('#spnTGT').text('');
            $('#spnInvLimit').text('');
            bindCustomer(BusinessSegment, $('#GroupID').val().trim(), $('#BRID').val().trim(), '0');
    ...
    $('#CUSTOMERID').change(function (e) {
        ...
        setTimeout(function () {
            ...
            if ($('#CUSTOMERID').val() != '0') {
                ...
                bindtaxcount();
                bindCategory($('#CUSTOMERID').val().trim());
                bindGTProduct($('#CUSTOMERID').val().trim(), $('#BRID').val().trim(), BusinessSegment, $('#CATID').val().trim(), $('#hdndispatchID').val().trim());
                ...
                /*Net Sale & TCS Limit*/
                NetSaleAmount($('#CUSTOMERID').val().trim(), $('#BRID').val().trim(), $('#hdnFinYear').val().trim(), $('#hdndispatchID').val().trim());
        ...
        setTimeout(function () {
            ...
            if (product != '0') {
                ...
                FreeProductBatchDetails($('#BRID').val().trim(), $('#FREEPRODUCTID').val().trim(), 'B9F29D12-DE94-40F1-A668-C79BF1BF4425',
                    ...
                    $('#GroupID').val().trim(), '0', '113BD8D6-E5DC-4164-BEE7-02A16F97ABCC');
            }
            else {
...
function loadAddNewRecord() {
    ...
    var closingstatus = ClosingStatus($('#InvoiceDate').val().trim(), $('#BRID').val().trim(), $("#hdnFinYear").val().trim());
    ...
    if (closingstatus != 'na') {
        $('#dvAdd').css("display", "none");
        $('#dvDisplay').css("display", "");
        toastr.warning('<b><font color=black>' + closingstatus + '</font></b>');
        return false;
    }
    else if (accpostingstatus != 'na') {
        $('#dvAdd').css("display", "none");
        $('#dvDisplay').css("display", "");
        ...
        setTimeout(function () {
            ...
            bindGroup(BusinessSegment, 'F', $("#hdnUserID").val().trim());
            ClearControls();
            //ReleaseSession();
            $("#imgLoader").css("visibility", "hidden");
            $("#dialog").dialog("close");
...
function bindCategory(customerid) {
    var Category = $("#CATID");
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetCategory",
...
function bindGTProduct(customerid, depotid, bsid, categoryid, invoiceid) {
    var Product = $("#PRODUCTID");

    $.ajax({
        type: "POST",
        url: "/TranDepot/GetGTProduct",
        data: { CustomerID: customerid, DepotID: depotid, BSID: bsid, CategoryID: categoryid, InvoiceID: invoiceid },
        async: false,
        dataType: "json",
        success: function (response) {
...
/*Net Sale & TCS Limit*/
function NetSaleAmount(customerid, depotid, finyear, invoiceid) {
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetNetSaleAmount",
        data: { CustomerID: customerid, DepotID: depotid, Finyear: finyear, InvoiceID: invoiceid  },
        async: false,
        dataType: "json",
...
function addProductInDetailsGrid() {
    ...
    stockcheckingmessage = getqtyinPCS($('#PRODUCTID').val().trim(), $('#PSID').val().trim(), $('#InvoiceQty').val().trim(), $('#InvoicePcs').val().trim(), $('#StockQty').val().trim(), '0', $('#hdndispatchID').val().trim())
    ...
    if (stockcheckingmessage != 'na') {
        ...
        if ($("#chkFree").prop('checked') == false) {
            if (Rate <= 0) {
                ...
                //debugger;
                priceschemediscount = getPriceSchemeDiscount($('#PRODUCTID').val().trim(), $("#hdninvoiceqty").val().trim(), 'B9F29D12-DE94-40F1-A668-C79BF1BF4425', $('#CUSTOMERID').val().trim(), '0', $('#InvoiceDate').val().trim(), BusinessSegment, $('#GroupID').val().trim(), $('#BRID').val().trim(), $('#MRP').val().trim());
                ...
                QuantityScheme($('#InvoiceDate').val().trim(), stateID, districtID, $('#PRODUCTID').val().trim(), $('#GroupID').val().trim(), BusinessSegment.trim(),
    ...
    if ($("#hdntaxcount").val().trim() == '1') {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                Amount = parseFloat(billQty * rate);
                //debugger;
                ...
                //}
                //else {
                //    IgstTaxAmount = parseFloat((Math.floor(((AfterDiscountAmount * IgstTaxPercentage) / 100) * 100) / 100));
                //}

                if (typeof IgstTaxPercentage === "undefined") {
                    IgstTaxPercentage = 0;
                    IgstTaxAmount = 0;
                }
                else {
                ...
                tr.append("<td style='text-align: right;'>" + parseFloat(SSMarginPercentage).toFixed(2) + "</td>");//9
                ...
                tr.append("<td style='text-align: right;'>" + parseFloat(SSMarginValue).toFixed(2) + "</td>");//16
                ...
                tr.append("<td style='text-align: right;'>" + parseFloat(discountvalue).toFixed(2) + "</td>");//19
                tr.append("<td style='text-align: right;'>" + parseFloat(DiscountValue).toFixed(2) + "</td>");//20
    ...
    /*CGST & SGST Start*/
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                //}
                //else {
                //    CgstTaxAmount = parseFloat((Math.floor(((AfterDiscountAmount * CgstTaxPercentage) / 100) * 100) / 100));
                //}

                if (typeof CgstTaxPercentage === "undefined") {
                        CgstTaxPercentage = 0;
                        CgstTaxAmount = 0;
                }
                 else {
                ...
                //}
                //else {
                //    SgstTaxAmount = parseFloat((Math.floor(((AfterDiscountAmount * SgstTaxPercentage) / 100) * 100) / 100));
                //}

                if (typeof SgstTaxPercentage === "undefined") {
                    SgstTaxPercentage = 0;
                    SgstTaxAmount = 0;
                }
                else {
...
function addProductInFreeGrid() {
    ...
    if (stockcheckingmessage != 'na') {
        ...
        if ($("#chkFree").prop('checked') == true) {
            ...
            tr.append("<td style='display: none;'>" + '0' + "</td>");//13
            if (productType == 'FG') {
                tr.append("<td style='text-align: right;'>" + parseFloat(Rate).toFixed(2) + "</td>");//14
                tr.append("<td style='text-align: right;'>" + parseFloat(Amount).toFixed(2) + "</td>");//15
            }
            else {
            ...
            if (productType == 'FG') {
                tr.append("<td style='display: none;'>" + parseFloat(Rate).toFixed(2) + "</td>");//22
            }
            else {
            ...
            tr.append("<td style='display: none;'>" + '0' + "</td>");//23
            if (productType == 'FG') {
                tr.append("<td style='text-align: right;'>" + parseFloat(Amount).toFixed(2) + "</td>");//24
            }
            else {
...
function bindGroup(bsid, checker,userid) {
    //debugger;
    var Group = $("#GroupID");
    var grouplength = 0;
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetGroup",
        data: { BSID: bsid, CheckerFlag: checker, UserID: userid },
        async: false,
        dataType: "json",
        success: function (response) {
            //debugger;
            grouplength = response.length;
            if (grouplength == 1) {
                Group.empty();
                $.each(response, function () {
                    //debugger;
                    Group.append($("<option></option>").val(this['DIS_CATID']).html(this['DIS_CATNAME']));
                    var GroupID = $('#GroupID').val().trim();
                    bindCustomer(BusinessSegment, GroupID.trim(), $('#BRID').val().trim(), '0');
                });
            }
                ...
                if ($("#BRID").val() == '0EEDDA49-C3AB-416A-8A44-0B9DFECD6670')/*Kolkta*/ {
                    $('#GroupID').val('AFF95D83-8297-40B3-A140-738F52D3F63C');/*GT*/
                    bindCustomer(BusinessSegment, $('#GroupID').val().trim(), $('#BRID').val().trim(), '0');
                }
...
function bindCustomer(bsid, groupid, depotid,invoiceid) {
    var Customer = $("#CUSTOMERID");
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetGTCustomer",
        data: { BSID: bsid, GroupID: groupid, DepotID: depotid, InvoiceID: invoiceid},
        async: false,
        dataType: "json",
        success: function (response) {
...
function getBatchDetails(focusflag) {
    ...
    bindRate($('#CUSTOMERID').val().trim(), $('#PRODUCTID').val().trim(), $('#InvoiceDate').val().trim(), $('#MRP').val().trim(), $('#BRID').val().trim(), menuid.trim(), BusinessSegment.trim(), $('#GroupID').val().trim(), 'F');
    if (focusflag == '0') {
        $("#InvoiceQty").focus();
    }
    else if (focusflag == '1') {
...
function addRowinTaxTable(taxID,TaxPercentage,TaxAmount, MRP,TaxFlag) {
    $.ajax({
        type: "POST",
        url: "/TranDepot/FillGTInvoiceTaxDatatable",
...
function deleteRowfromTaxTable(grdproductid, grdbatchno) {
    $.ajax({
        type: "POST",
        url: "/TranDepot/DeleteTaxDatatableGTInvoice",
...
function RowCountGtInvoiceList() {
    var table = document.getElementById("GTInvoiceGrid");
    var rowCount = document.getElementById("GTInvoiceGrid").rows.length - 1;
    if (rowCount > 0) {
...
function CalculateAmount() {
    ...
    if (rowCount > 0 && freerowCount > 0) {
        ...
        /*Round Up Logic start from 28-08-2020*/
        if (decimalvalue > 0) {
            decimalvalue = 1 - decimalvalue;
            decimalvalue = decimalvalue.toFixed(2);
        }
...
function ClearControls() {
    ...
    var Grouplength = $('#GroupID > option').length;
    if ($("#BRID").val() != '0EEDDA49-C3AB-416A-8A44-0B9DFECD6670')/*Kolkata*/ {
...
function SaveInvoice() {
    ...
    if (returnflag == true && Taxreturnflag==true) {
        ...
        BillingBalance = parseFloat($('#ClosingBalance').val().trim()) + parseFloat($('#CreditLimit').val().trim());
        if (parseFloat(BillingBalance) < parseFloat($('#NetAmt').val().trim())) {

            toastr.error('<b>Insufficient Balance...!</b>');
            return false;
        }
        else {
            ...
            //debugger;
            gtinvoicesave.FGInvoiceID = $('#hdndispatchID').val().trim();

            if ($('#hdndispatchID').val() == '0') {
                gtinvoicesave.FLAG = 'A';
            }
            else {
                gtinvoicesave.FLAG = 'U';
            }
            gtinvoicesave.InvoiceDate = $("#InvoiceDate").val().trim();
            gtinvoicesave.CUSTOMERID = $("#CUSTOMERID").val().trim();
            gtinvoicesave.CUSTOMERNAME = $("#CUSTOMERID option:selected").text().trim();
            gtinvoicesave.WAYBILLNO = '0';
            gtinvoicesave.TransporterID = $("#TransporterID").val().trim();
            if ($("#VehichleNo").val().trim() == '') {
                gtinvoicesave.VehichleNo = '';
            }
            else {
                gtinvoicesave.VehichleNo = $("#VehichleNo").val().trim();
            }
            gtinvoicesave.BRID = $("#BRID").val().trim();
            gtinvoicesave.BRNAME = $("#BRID option:selected").text().trim();
            if ($("#LrGrNo").val().trim() == '') {
                gtinvoicesave.LrGrNo = '';
            }
            else {
                gtinvoicesave.LrGrNo = $("#LrGrNo").val().trim();
            }
            gtinvoicesave.LrGrDate = $("#LrGrDate").val().trim();
            if ($("#GatepassNo").val().trim() == '') {
                gtinvoicesave.GatepassNo = '';
            }
            else {
                gtinvoicesave.GatepassNo = $("#GatepassNo").val().trim();
            }
            gtinvoicesave.GatepassDate = $("#GatepassDate").val().trim();
            gtinvoicesave.PaymentMode = $("#PaymentMode").val().trim();
            gtinvoicesave.Tranportmode = $("#Tranportmode").val().trim();
            if ($("#Remarks").val().trim() == '') {
                gtinvoicesave.Remarks = '';
            }
            else {
                gtinvoicesave.Remarks = $("#Remarks").val().trim();
            }
            gtinvoicesave.NetAmt = $("#NetAmt").val().trim();
            gtinvoicesave.RoundOff = $("#RoundOff").val().trim();
            gtinvoicesave.TotalFreeAmt = $("#TotalFreeAmt").val().trim();
            gtinvoicesave.GroupID = $("#GroupID").val().trim();
            gtinvoicesave.GroupName = $("#GroupID option:selected").text().trim();
            gtinvoicesave.TotalPcs = $("#TotalPcs").val().trim();
            gtinvoicesave.SSMargin = $("#SSMargin").val().trim();
            gtinvoicesave.SSMarginAmt = $("#SSMarginAmt").val().trim();
            gtinvoicesave.TotalCase = $("#TotalCase").val().trim();
            if ($('#hdntaxcount').val() == '1') {
                gtinvoicesave.InvoiceType = '1';
            }
            else if ($('#hdntaxcount').val() == '2') {
                gtinvoicesave.InvoiceType = '2';
            }
            else if ($('#hdntaxcount').val() == '3') {
                gtinvoicesave.InvoiceType = '3';
            }
            else {
                gtinvoicesave.InvoiceType = '0';
            }
            if ($("#TotalGrossWght").val().trim() == '') {
                gtinvoicesave.TotalGrossWght = '0';
            }
            else {
                gtinvoicesave.TotalGrossWght = $("#TotalGrossWght").val().trim();
            }
            gtinvoicesave.ReferencePO = $("#ReferencePO").val().trim();
            gtinvoicesave.BasicAmt = $("#BasicAmt").val().trim();
            gtinvoicesave.MonthID = Month.trim();

            gtinvoicesave.UserID = $("#hdnUserID").val().trim();
            gtinvoicesave.FinYear = $("#hdnFinYear").val().trim();
            if (freerowCount <= 0) {
                gtinvoicesave.GTFreeTag = '0';
            }
            else {
                gtinvoicesave.GTFreeTag = '1';
            }

            /*TCS Controls*/
            gtinvoicesave.TCSPercent = $("#TCSPercent").val().trim();
            gtinvoicesave.TCSAmt = $("#TCSAmt").val().trim();
            gtinvoicesave.TCSNetAmt = $("#TCSNetAmt").val().trim();
            ...
            gtinvoicesave.InvoiceDetailsGT = invoicelist;
            ...
            gtinvoicesave.FreeDetailsGT = freelist;
            ...
            $('#taxDetailsGrid tbody tr').each(function () {
                ...
                taxlist[k++] = gttaxdetails;
            });

            gtinvoicesave.TaxDetailsGT = taxlist;

            //alert(JSON.stringify(invoicesave));

            $.ajax({
                url: "/TranDepot/gtInvoicesavedata",
                data: '{gtinvoicesave:' + JSON.stringify(gtinvoicesave) + '}',
                type: "POST",
                async: false,
...
function ReleaseSession() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepot/RemoveSessionGTInvoice",
        data: '{}',
        dataType: "json",
        success: function (response) {
        }
...
function bindgtinvoicegrid() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepot/BindGtInvoiceGrid",
        ...
        success: function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                if (response[i].INTRANSITDESC.trim() == 'Received') {
                    tr.append("<td style='text-align: center'><font color='0x00B300'>" + response[i].INTRANSITDESC + "</font></td>");
                }
                else {
                    tr.append("<td style='text-align: center'><font color='#ff2500'>" + response[i].INTRANSITDESC + "</font></td>");
                }
                ...
                tr.append("<td style='text-align: center'><input type='image' class='gvGTPrint'  id='btngtprint'  <img src='../Images/ico_Print.png' width='20' height ='20' title='Print'/></input></td>");
                tr.append("<td style='text-align: center'><input type='image' class='gvGTEdit'   id='btngtedit'   <img src='../Images/Pencil-icon.png' title='Edit'/></input></td>");
                tr.append("<td style='text-align: center'><input type='image' class='gvGTView'   id='btngtview'   <img src='../Images/View.png' width='20' height ='20' title='View'/></input></td>");
                tr.append("<td style='text-align: center'><input type='image' class='gvGTCancel' id='btngtdelete' <img src='../Images/ico_delete_16.png' title='Cancel'/></input></td>");
                
                $("#GTInvoiceGrid").append(tr);
            }
            tr.append("</tbody>");
            RowCountGtInvoiceList();
            $('#GTInvoiceGrid').DataTable({
                "sScrollX": '100%',
                "sScrollXInner": "110%",
                "scrollY": "238px",
                "scrollCollapse": true,
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $.each(listHeader, function (index, record) {
                ...
                Group.empty();
                Group.append($("<option></option>").val(this['GROUPID']).html(this['GROUPNAME']));
            ...
            if (listDetails.length > 0) {
                ...
                if ($("#hdntaxcount").val().trim() == '1') {
                    ...
                    $.each(listDetails, function (index, record) {
                        ...
                        tr.append("<td style='text-align: right;'>" + parseFloat(this['RATEDISC'].toString().trim()).toFixed(2) + "</td>");//9
                        ...
                        tr.append("<td style='text-align: right;'>" + parseFloat(this['RATEDISCVALUE'].toString().trim()).toFixed(2) + "</td>");//16
                        ...
                        tr.append("<td style='text-align: right;'>" + parseFloat(this['DISCPER'].toString().trim()).toFixed(2) + "</td>");//19
                        tr.append("<td style='text-align: right;'>" + parseFloat(this['DISCAMT'].toString().trim()).toFixed(2) + "</td>");//20
                        ...
                        IgstAmount = parseFloat((Math.floor(((this['AMOUNT'].toString().trim() * IgstPercentage) / 100) * 100) / 100));
                    ...
                    $.each(listDetails, function (index, record) {
                        ...
                        CgstAmount = parseFloat((Math.floor(((this['AMOUNT'].toString().trim() * CgstPercentage) / 100) * 100) / 100));
                        SgstPercentage = GetTaxOnEdit(invoiceid.trim(), Sgstid, this['PRODUCTID'].toString().trim(), this['BATCHNO'].toString().trim());
                        SgstAmount = parseFloat((Math.floor(((this['AMOUNT'].toString().trim() * SgstPercentage) / 100) * 100) / 100));
            ...
            if (listFooter.length > 0) {
                $.each(listFooter, function (index, record) {
                    ...
                    $("#SSMargin").val(parseFloat(this['ADDSSMARGINPERCENTAGE'].toString().trim()).toFixed(2));
                    $("#SSMarginAmt").val(parseFloat(this['ADDSSMARGINAMT'].toString().trim()).toFixed(2));
...
$(function () {
    var invoiceid;
    $("body").on("click", "#GTInvoiceGrid .gvGTEdit", function () {
        var row = $(this).closest("tr");
...
$(function () {
    var invoiceid;
    $("body").on("click", "#GTInvoiceGrid .gvGTView", function () {
        var row = $(this).closest("tr");
        invoiceid = row.find('td:eq(1)').html();
        $('#hdndispatchID').val(invoiceid);
...
$(function () {
    //debugger;
    var dispatchid;
    $("body").on("click", "#GTInvoiceGrid .gvGTCancel", function () {
        var row = $(this).closest("tr");
...
$(function () {
    ...
    $("body").on("click", "#GTInvoiceGrid .gvGTPrint", function () {
        var row = $(this).closest("tr");
        invoiceid = row.find('td:eq(1)').html();
        $('#hdndispatchID').val(invoiceid);
        $("#dialog").dialog({
