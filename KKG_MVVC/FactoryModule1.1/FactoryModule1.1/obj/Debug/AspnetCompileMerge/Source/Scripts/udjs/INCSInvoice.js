...
function addRowinTaxTable(taxID, TaxPercentage, TaxAmount, MRP, TaxFlag) {
    $.ajax({
        type: "POST",
        url: "/TranDepot/FillINCSInvoiceTaxDatatable",
...
function deleteRowfromTaxTable(grdproductid, grdbatchno) {
    $.ajax({
        type: "POST",
        url: "/TranDepot/DeleteTaxDatatableINCSInvoice",
...
function RowCountCsdInvoiceList() {
    var table = document.getElementById("INCSInvoiceGrid");
    var rowCount = document.getElementById("INCSInvoiceGrid").rows.length - 1;
    if (rowCount > 0) {
    ...
    if (returnflag == true && Taxreturnflag == true) {
        ...
        if (saveflag == true) {
            ...
            setTimeout(function () {
                ...
                csdinvoicesave.ICDSDate = '01/01/1900';
                ...
                $.ajax({
                    url: "/TranDepot/incsInvoicesavedata",
                    data: '{incsinvoicesave:' + JSON.stringify(csdinvoicesave) + '}',
                    type: "POST",
                    async: false,
...
function ReleaseSession() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepot/RemoveSessionINCSInvoice",
        data: '{}',
        dataType: "json",
        success: function (response) {
        }
...
function bindcsdinvoicegrid() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#INCSInvoiceGrid').DataTable().destroy();
            $("#INCSInvoiceGrid tbody tr").remove();
            for (var i = 0; i < response.length; i++) {
                ...
                $("#INCSInvoiceGrid").append(tr);
            }
            tr.append("</tbody>");
            RowCountCsdInvoiceList();
            $('#INCSInvoiceGrid').DataTable({
                "sScrollX": '100%',
                "sScrollXInner": "110%",
                "scrollY": "238px",
                "scrollCollapse": true,
...
$(function () {
    var invoiceid;
    $("body").on("click", "#INCSInvoiceGrid .gvCSDEdit", function () {
        var row = $(this).closest("tr");
...
$(function () {
    var invoiceid;
    $("body").on("click", "#INCSInvoiceGrid .gvCSDView", function () {
        var row = $(this).closest("tr");
        invoiceid = row.find('td:eq(1)').html();
        $('#hdndispatchID').val(invoiceid);
...
$(function () {
    //debugger;
    var dispatchid;
    $("body").on("click", "#INCSInvoiceGrid .gvCSDCancel", function () {
        var row = $(this).closest("tr");
...
$(function () {
    var invoiceid;
    var acstatus = '';
    $("body").on("click", "#INCSInvoiceGrid .gvCSDPrint", function () {
        var row = $(this).closest("tr");
        invoiceid = row.find('td:eq(1)').html();
        $('#hdndispatchID').val(invoiceid);
