...
/*
* For FactoryInvoiceExport.cshtml Only
...
function finyearChecking() {
    ...
    backdatestatus = bindbackdateflag('4215');
...
function addProductInDetailsGrid() {
    ...
    $.ajax({
        ...
        data: { Productid: $('#PRODUCTID').val().trim(), PacksizeID: $('#PSID').val().trim(), Qty: $('#InvoiceQty').val().trim(), Rate: parseFloat(Rate), Assesment: $('#hdn_ASSESMENTPERCENTAGE').val().trim(), TaxName: 'NA', date: $('#InvoiceDate').val().trim() },
        async: false,
        dataType: "json",
        success: function (response) {
            debugger;
            ...
            tr.append("<td style='display: none'>1</td>");//24
    ...
    addRowinTaxTable($('#PRODUCTID').val().trim(), 'NA', 'NA', IgstTaxPercentage, IgstTaxAmount, MRP, '0');
    /*********************/
    clearafterAdd();
    $("#PRODUCTID").focus();
...
function addRowinTaxTable(ProductID, batch, taxid, TaxPercentage, TaxAmount, MRP, TaxFlag) {
    $.ajax({
        type: "POST",
        url: "/tranfac/FillExportTaxDatatableNormal",
...
function deleteRowfromTaxTable(grdproductid, grdbatchno) {
    $.ajax({
        type: "POST",
        url: "/tranfac/DeleteTaxDatatableExportNormal",
...
function CalculateAmount() {
    ...
    if (rowCount > 0 && freerowCount > 0) {
        ...
        $('#productDetailsGrid tbody tr').each(function () {
            totalbasicamt += parseFloat($(this).find('td:eq(18)').html().trim());
            totalbasicplustaxamt += parseFloat($(this).find('td:eq(18)').html().trim());
...
function SaveInvoice() {
    ...
    $('#productDetailsGrid tbody tr').each(function () {
        ...
        var tag = $(this).find('td:eq(24)').html().trim();
...
function ReleaseSession() {
    ...
    $.ajax({
        type: "POST",
        url: "/tranfac/RemoveSessionExportNormal",
        data: '{}',
        dataType: "json",
        success: function (response) {
...
function bindexportinvoicegrid() {
    ...
    $.ajax({
        ...
        data: { FromDate: $('#txtfrmdate').val().trim(), ToDate: $('#txttodate').val().trim(), BSID: BusinessSegment, depotID: $('#BRID').val().trim(), type: '0' },
        dataType: "json",
        success: function (response) {
            debugger;
            var tr;
        ...
        success: function (response) {
            ...
            if (listDetails.length > 0) {
                ...
                $.each(listDetails, function (index, record) {
                    ...
                    NetAmount = (parseFloat(this['AMOUNTINR'].toString().trim()));
                    tr.append("<td style='text-align: right'>" + NetAmount.toFixed(2) + "</td>");//23
                    tr.append("<td style='display: none'>" + this['TAG'].toString().trim() + "</td>");//24
                    tr.append("<td style='text-align: center'><input type='image' class='gvTempDelete'  id='btndelete' <img src='../Images/ico_delete_16.png'/></input></td>");
                    $("#productDetailsGrid").append(tr);
                });
                tr.append("</tbody>");
        ...
        success: function (response) {
            ...
            if (response.length > 0) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        ...
                        NetAmount = (parseFloat(response[i].AMOUNTINR.trim()));
                        tr.append("<td style='text-align: right'>" + NetAmount.toFixed(2) + "</td>");//23
                        tr.append("<td style='display: none'>0</td>");//24
                        tr.append("<td style='text-align: center'><input type='image' class='gvTempDelete'  id='btndelete' <img src='../Images/ico_delete_16.png'/></input></td>");
                        ...
                        /*Tax Details Info*/
                        addRowinTaxTableEdit(response[i].PRODUCTID.trim(), response[i].BATCHNO.trim(), '1177D9CF-8F1E-4C91-B785-FDF940101EEE', IgstPercentage, IgstAmount, response[i].MRP.trim(), '0');
                    }
                    tr.append("</tbody>");
                    RowCount();
                    CalculateAmount();
