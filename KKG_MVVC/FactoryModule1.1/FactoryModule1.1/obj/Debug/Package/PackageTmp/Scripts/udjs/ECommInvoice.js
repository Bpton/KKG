...
$(document).ready(function () {
    ...
    bindPacksize();
    bindecomminvoicegrid();
    bindCategory();
    bindEcommProduct(BusinessSegment, $('#CATID').val().trim(), $('#hdndispatchID').val().trim());
    ...
    $('#TransporterID').change(function (e) {
        ...
        //$("#Tranportmode").focus();
        //$('#Tranportmode').trigger('chosen:activate');
        $("#CUSTOMERID").focus();
...
function bindCategory() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetEcommCategory",
        data: '{}',
        async: false,
        dataType: "json",
...
function bindEcommProduct(bsid, categoryid, invoiceid) {
    var Product = $("#PRODUCTID");

    $.ajax({
        type: "POST",
        url: "/TranDepot/GetECommProduct",
        data: { BSID: bsid, CategoryID: categoryid, InvoiceID: invoiceid},
        async: false,
        dataType: "json",
        success: function (response) {
    ...
    if ($("#hdntaxcount").val().trim() == '1') {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                tr.append("<td style='text-align: right;display: none;'>" + parseFloat(SSMarginPercentage).toFixed(2) + "</td>");//9
...
function addRowinTaxTable(taxID,TaxPercentage,TaxAmount, MRP,TaxFlag) {
    $.ajax({
        type: "POST",
        url: "/TranDepot/FillECOMMInvoiceTaxDatatable",
...
function deleteRowfromTaxTable(grdproductid, grdbatchno) {
    $.ajax({
        type: "POST",
        url: "/TranDepot/DeleteTaxDatatableECOMMInvoice",
...
function RowCountGtInvoiceList() {
    var table = document.getElementById("EcommInvoiceGrid");
    var rowCount = document.getElementById("EcommInvoiceGrid").rows.length - 1;
    if (rowCount > 0) {
...
function SaveInvoice() {
    ...
    if (returnflag == true && Taxreturnflag == true) {
        ...
        if (parseFloat(BillingBalance) < parseFloat($('#NetAmt').val().trim())) {
            ...
            freelist = new Array();
            taxlist = new Array();
            var ecomminvoicesave = {};

            debugger;
                ecomminvoicesave.FGInvoiceID = $('#hdndispatchID').val().trim();

            if ($('#hdndispatchID').val() == '0') {
                ecomminvoicesave.FLAG = 'A';
            }
            else {
                ecomminvoicesave.FLAG = 'U';
            }
            ecomminvoicesave.InvoiceDate = $("#InvoiceDate").val().trim();
            ecomminvoicesave.CUSTOMERID = $("#CUSTOMERID").val().trim();
            ecomminvoicesave.CUSTOMERNAME = $("#CUSTOMERID option:selected").text().trim();
            ecomminvoicesave.WAYBILLNO = '0';
            ecomminvoicesave.TransporterID = $("#TransporterID").val().trim();
            if ($("#VehichleNo").val().trim() == '') {
                ecomminvoicesave.VehichleNo = '';
            }
            else {
                ecomminvoicesave.VehichleNo = $("#VehichleNo").val().trim();
            }
            ecomminvoicesave.BRID = $("#BRID").val().trim();
            ecomminvoicesave.BRNAME = $("#BRID option:selected").text().trim();
            if ($("#LrGrNo").val().trim() == '') {
                ecomminvoicesave.LrGrNo = '';
            }
            else {
                ecomminvoicesave.LrGrNo = $("#LrGrNo").val().trim();
            }
            ecomminvoicesave.LrGrDate = $("#LrGrDate").val().trim();
            if ($("#GatepassNo").val().trim() == '') {
                ecomminvoicesave.GatepassNo = '';
            }
            else {
                ecomminvoicesave.GatepassNo = $("#GatepassNo").val().trim();
            }
            ecomminvoicesave.GatepassDate = $("#GatepassDate").val().trim();
            ecomminvoicesave.PaymentMode = $("#PaymentMode").val().trim();
            ecomminvoicesave.Tranportmode = $("#Tranportmode").val().trim();
            if ($("#Remarks").val().trim() == '') {
                ecomminvoicesave.Remarks = '';
            }
            else {
                ecomminvoicesave.Remarks = $("#Remarks").val().trim();
            }
            ecomminvoicesave.NetAmt = $("#NetAmt").val().trim();
            ecomminvoicesave.RoundOff = $("#RoundOff").val().trim();
            ecomminvoicesave.TotalFreeAmt = $("#TotalFreeAmt").val().trim();
            ecomminvoicesave.GroupID = $("#GroupID").val().trim();
            ecomminvoicesave.GroupName = $("#GroupID option:selected").text().trim();
            ecomminvoicesave.TotalPcs = $("#TotalPcs").val().trim();
            ecomminvoicesave.SSMargin = $("#SSMargin").val().trim();
            ecomminvoicesave.SSMarginAmt = $("#SSMarginAmt").val().trim();
            ecomminvoicesave.TotalCase = $("#TotalCase").val().trim();
            if ($('#hdntaxcount').val() == '1') {
                ecomminvoicesave.InvoiceType = '1';
            }
            else if ($('#hdntaxcount').val() == '2') {
                ecomminvoicesave.InvoiceType = '2';
            }
            else if ($('#hdntaxcount').val() == '3') {
                ecomminvoicesave.InvoiceType = '3';
            }
            else {
                ecomminvoicesave.InvoiceType = '0';
            }
            if ($("#TotalGrossWght").val().trim() == '') {
                ecomminvoicesave.TotalGrossWght = '0';
            }
            else {
                ecomminvoicesave.TotalGrossWght = $("#TotalGrossWght").val().trim();
            }
            ecomminvoicesave.ReferencePO = $("#ReferencePO").val().trim();
            ecomminvoicesave.BasicAmt = $("#BasicAmt").val().trim();
            ecomminvoicesave.MonthID = Month.trim();

            ecomminvoicesave.UserID = $("#hdnUserID").val().trim();
            ecomminvoicesave.FinYear = $("#hdnFinYear").val().trim();
            if (freerowCount <= 0) {
                ecomminvoicesave.GTFreeTag = '0';
            }
            else {
                ecomminvoicesave.GTFreeTag = '1';
            }

            ecomminvoicesave.ShippingAddress = $("#ShippingAddress").val().trim();
            /*TCS Controls*/
            ecomminvoicesave.TCSPercent = $("#TCSPercent").val().trim();
            ecomminvoicesave.TCSAmt = $("#TCSAmt").val().trim();
            ecomminvoicesave.TCSNetAmt = $("#TCSNetAmt").val().trim();
            ...
            ecomminvoicesave.InvoiceDetailsGT = invoicelist;
            ...
            ecomminvoicesave.FreeDetailsGT = freelist;
            ...
            ecomminvoicesave.TaxDetailsGT = taxlist;

            //alert(JSON.stringify(invoicesave));

            $.ajax({
                url: "/TranDepot/ecommInvoicesavedata",
                data: '{ecomminvoicesave:' + JSON.stringify(ecomminvoicesave) + '}',
                type: "POST",
                async: false,
...
function ReleaseSession() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepot/RemoveSessionECOMMInvoice",
        data: '{}',
        dataType: "json",
        success: function (response) {
        }
...
function bindecomminvoicegrid() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#EcommInvoiceGrid').DataTable().destroy();
            $("#EcommInvoiceGrid tbody tr").remove();
            for (var i = 0; i < response.length; i++) {
                ...
                tr.append("<td style='text-align: center'><input type='image' class='gvEcommPrint'  id='btnecommprint'  <img src='../Images/ico_Print.png' width='20' height ='20' title='Print'/></input></td>");
                tr.append("<td style='text-align: center'><input type='image' class='gvEcommEdit'   id='btnecommedit'   <img src='../Images/Pencil-icon.png' title='Edit'/></input></td>");
                tr.append("<td style='text-align: center'><input type='image' class='gvEcommView'   id='btnecommview'   <img src='../Images/View.png' width='20' height ='20' title='View'/></input></td>");
                tr.append("<td style='text-align: center'><input type='image' class='gvEcommCancel' id='btnecommdelete' <img src='../Images/ico_delete_16.png' title='Cancel'/></input></td>");
                
                $("#EcommInvoiceGrid").append(tr);
            }
            tr.append("</tbody>");
            RowCountGtInvoiceList();
            $('#EcommInvoiceGrid').DataTable({
                "sScrollX": '100%',
                "sScrollXInner": "110%",
                "scrollY": "238px",
                "scrollCollapse": true,
            ...
            if (listDetails.length > 0) {
                ...
                if ($("#hdntaxcount").val().trim() == '1') {
                    ...
                    $.each(listDetails, function (index, record) {
                        ...
                        tr.append("<td style='text-align: right;display: none;'>" + parseFloat(this['RATEDISC'].toString().trim()).toFixed(2) + "</td>");//9
...
$(function () {
    var invoiceid;
    $("body").on("click", "#EcommInvoiceGrid .gvEcommEdit", function () {
        var row = $(this).closest("tr");
...
$(function () {
    var invoiceid;
    $("body").on("click", "#EcommInvoiceGrid .gvEcommView", function () {
        var row = $(this).closest("tr");
        invoiceid = row.find('td:eq(1)').html();
        $('#hdndispatchID').val(invoiceid);
...
$(function () {
    debugger;
    var dispatchid;
    $("body").on("click", "#EcommInvoiceGrid .gvEcommCancel", function () {
        var row = $(this).closest("tr");
...
$(function () {
    ...
    $("body").on("click", "#EcommInvoiceGrid .gvEcommPrint", function () {
        var row = $(this).closest("tr");
        invoiceid = row.find('td:eq(1)').html();
        $('#hdndispatchID').val(invoiceid);
        $("#dialog").dialog({
