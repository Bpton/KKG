...
/*
* For StockAdjustment.cshtml Only
...
$(document).ready(function () {
    ...
    $('#divBatchSearch').css("display", "none");
    ...
    bindproduct($("#BRID").val().trim(), $("#CATID").val().trim(), Mode);
            ...
            if ($('#StorelocationID').val() != '0') {
                ...
                setTimeout(function () {
                    ...
                    if ($("#radio_General").prop("checked")) {
                        ...
                        var Type = JournalProductType($('#PRODUCTID').val().trim());
                        if (Type == '1') {
                    ...
                    else if ($("#radio_Exception").prop("checked")) {
    ...
    $('#CATID').change(function (e) {
        ...
        setTimeout(function () {
            if ($("#radio_General").prop("checked")) {
            bindproduct($("#BRID").val().trim(), $("#CATID").val().trim(), $("#radio_General").val().trim());
            }
            else if ($("#radio_Exception").prop("checked")) {
            bindproduct($("#BRID").val().trim(), $("#CATID").val().trim(), $("#radio_Exception").val().trim());
            }
            $("#PRODUCTID").chosen({
                search_contains: true
            });
    ...
    $("#btnadd").click(function (e) {
        ...
        if ($("#radio_Exception").prop("checked")) {
        ...
        if (ExcessFlag == '1') {
            if ($('#PRODUCTID').val() == '0') {
                ...
                toastr.warning('<b><font color=black>Journal quantity should be greater than 0..!</font></b>');
                AddFlag = false;
                return false;
            }
        }
        else if (ExcessFlag == '0') {
            if ($('#PRODUCTID').val() == '0') {
                ...
                if ((parseInt($('#JournalQuantity').val()) * -1) > parseInt($('#StockQty').val())) {

                         toastr.warning('<b><font color=black>Journal quantity should not greater than Stock quantity..!</font></b>');
                         AddFlag = false;
                         return false;
    ...
    $("#radio_Exception").click(function () {
        $("#dialog").dialog({
            autoOpen: true,
            modal: true,
            title: "Loading.."
...
function IsExistsJournalProduct(ProductId, ProductName, BatchNo, Mrp, PacksizeID, PacksizeName, JournalQty,
    ...
    if ($('#JournalDetailsGrid').length) {
        ...
        for (i = 0; i < jsondispatchobj.conversionproductDetails.length; i++) {
            if (jsondispatchobj.conversionproductDetails[i].INPUTPRODUCT.trim() == ProductId.trim() && jsondispatchobj.conversionproductDetails[i].INPUTBATCH.trim() == BatchNo.trim()) {
                exists = true;
                break;
            }
        }
...
function CalculateJournalQty() {
    
    var totalpositiveqty = 0;
    var totalnegativeqty = 0;
    var rowCount = 0;
    rowCount = document.getElementById("JournalDetailsGrid").rows.length - 1;
    ...
    if (rowCount > 0) {
        ...
        $('#JournalDetailsGrid tbody tr').each(function () {
            ...
            debugger;
            if (parseInt($(this).find('td:eq(7)').html().trim()) > 0) {
                totalpositiveqty += parseInt($(this).find('td:eq(7)').html().trim());
            }
            else if (parseInt($(this).find('td:eq(7)').html().trim()) < 0) {
                totalnegativeqty += parseInt($(this).find('td:eq(7)').html().trim());
            }

        });
        $('#TotalStockQty').val(totalpositiveqty);
        $('#TotalConversionQty').val('-'+ totalnegativeqty);
    }
    else {
...
function bindCategory() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetJournalCategory",
        data: '{}',
        async: false,
        dataType: "json",
        success: function (response) {
...
function bindproduct(depotid,categoryid,mode) {
    debugger;
    var Product = $("#PRODUCTID");
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetJournalProduct",
        data: { DepotID: depotid, CategoryID: categoryid, Mode: mode },
        async: false,
        dataType: "json",
        success: function (response) {
            //debugger;
...
function bindRMPMbatchno() {
    ...
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetJournalRMPMBatchDetails",
        data: { DepotID: $('#BRID').val(), ProductID: $('#PRODUCTID').val(), BatchNo: Batch, StorelocationID: $('#StorelocationID').val() },
        dataType: "json",
        success: function (batchlist) {
...
function bindExcessbatchno() {
    debugger;
    var ddlBatch = $("#ddlBatch");
    var Batch = '0';
    var fromdate = '';
    var todate = '';
    if ($("#BatchFromDate").val()!='') {

        fromdate = $("#BatchFromDate").val().trim();
    }
    if ($("#BatchToDate").val() != '') {

        todate = $("#BatchToDate").val().trim();
    }
    var desc = '';
    var desc1 = '';
    var val1 = '';
    var listItems = '';
    listItems = "<option value='0'>Select Batch</option>";
    desc = 'MRP'.padEnd(9, '\u00A0') + repeatstr("&nbsp;", 9) + 'Mfg.Date'.padEnd(10, '\u00A0') + repeatstr("&nbsp;", 10) + 'Exp.Date'.padEnd(12, '\u00A0') + repeatstr("&nbsp;", 12) + 'Batch'.padEnd(15, '\u00A0');
    listItems += "<option value='0'>" + desc + "</option>";
    
    $.ajax({
        type: "POST",
        url: "/TranDepot/GetExcessBatchDetails",
        data: { DepotID: $('#BRID').val(), ProductID: $('#PRODUCTID').val(), Packsize: 'B9F29D12-DE94-40F1-A668-C79BF1BF4425', BatchNo: Batch, FromDate: fromdate, ToDate: todate, MRP: '0', StorelocationID: $('#StorelocationID').val() },
        dataType: "json",
        success: function (excessbatchlist) {

            debugger;
            var counter = 0;
            $('#ddlBatch').empty();
            $.each(excessbatchlist, function () {
                val1 = this["BATCHNO"] + "|" + this["BatchMRP"] + "|" + this["BatchMFGDATE"] + "|" + this["BatchEXPIRDATE"];
                desc1 = this["BatchMRP"].padEnd(9, '\u00A0') + repeatstr("&nbsp;", 9) + this["BatchMFGDATE"].padEnd(10, '\u00A0') + repeatstr("&nbsp;", 10) + this["BatchEXPIRDATE"].padEnd(12, '\u00A0') + repeatstr("&nbsp;", 12) + this["BATCHNO"].padEnd(15, '\u00A0')
                listItems += "<option value='" + val1 + "'>" + desc1 + "</option>";
                counter = counter + 1;
            });
            ddlBatch.append(listItems);
        ...
        failure: function (excessbatchlist) {
            alert(excessbatchlist.responseText);
        },
        error: function (excessbatchlist) {
            alert(excessbatchlist.responseText);
        }
    });
}
...
function ClearControls() {
    ...
    $("#CheckerRemarks").attr("disabled", "disabled");
...
function bindstockjournalgrid() {
    ...
    $.ajax({
        ...
        success: function (response) {
            var tr;
            tr = $('#StockJournalGrid');
            tr.append("<thead><tr><th>Sl. No.</th><th style='display: none'>ADJUSTMENTID</th><th>Journal Date</th><th>Journal No</th><th>Depot</th><th>Journal Type</th><th>Entry User</th><th>Dayend Status</th><th>Approval Status</th><th>Print</th><th>View</th>");
            ...
            for (var i = 0; i < response.length; i++) {
                ...
                tr.append("<td >" + response[i].DEPOTNAME + "</td>");
                tr.append("<td >" + response[i].ADJUSTMENT_FROMMENU + "</td>");
                ...
                if (response[i].APPROVED_STATUS.trim() == 'Approved') {
                    tr.append("<td style='text-align: center'><font color='0x00B300'>" + response[i].APPROVED_STATUS + "</font></td>");
                }
                else if (response[i].APPROVED_STATUS.trim() == 'Pending') {
                    tr.append("<td style='text-align: center'><font color='0x5184FF'>" + response[i].APPROVED_STATUS + "</font></td>");
                }
                else {
                    tr.append("<td style='text-align: center'><font color='#ff2500'>" + response[i].APPROVED_STATUS + "</font></td>");
                }
                tr.append("<td style='text-align: center'><input type='image' class='gvJournalPrint'  id='btnjournalprint'  <img src='../Images/ico_Print.png' width='20' height ='20' title='Print'/></input></td>");
                tr.append("<td style='text-align: center'><input type='image' class='gvJournalView'   id='btnjournalview'   <img src='../Images/View.png' width='20' height ='20' title='View'/></input></td>");
