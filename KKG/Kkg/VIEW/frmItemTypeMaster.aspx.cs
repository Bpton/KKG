...
public partial class VIEW_frmItemTypeMaster : System.Web.UI.Page
{
    ...
    public void LoadItemType()
    {
        try
        {
            gvittype.DataSource = ClsITMaster.BindItemTypeMasterGrid();
            gvittype.DataBind();
        }
        ...
        {
            {
                ...
                ID = ClsITMaster.SaveItemTypeMasterGrid(Hdn_Fld.Value, txtITCode.Text.ToString(), txtITName.Text.ToString(), txtDescription.Text.ToString(), Mode, chkActive.Checked.ToString());
    ...
    protected void DeleteRecord(object sender, Obout.Grid.GridRecordEventArgs e)
    {
        try
        {
            ...
            {
                int ID = 0;
                ID = ClsITMaster.DeleteITMaster(e.Record["ITID"].ToString());
                if (ID > 0)
                {
