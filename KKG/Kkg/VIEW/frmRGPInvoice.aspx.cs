...
public partial class VIEW_frmRGPInvoice : System.Web.UI.Page
{
    ...
    public void Loadparty()
    {
        try
        {
            ...
            dtPrimary = ClsSaleorderFG.bindparty();
            if (dtPrimary.Rows.Count > 0)
            {
                ddlparty.Items.Clear();
                ddlparty.Items.Insert(0, new ListItem("--Select Party--", "0"));
                this.ddlparty.AppendDataBoundItems = true;
                ddlparty.DataSource = dtPrimary;
    ...
    public void Loadtransporter()
    {
        try
        {
            ClsSaleorderFG ClsSaleorderFG = new ClsSaleorderFG();
            DataTable dtPrimary = new DataTable();
            dtPrimary = ClsSaleorderFG.bindtransporter();
            if (dtPrimary.Rows.Count > 0)
            {
                ddltransport.Items.Clear();
                ddltransport.Items.Insert(0, new ListItem("--Select Transporter--", "0"));
                this.ddltransport.AppendDataBoundItems = true;
                ddltransport.DataSource = dtPrimary;
    ...
    public void LoadSale()
    {
        ...
        {
            ClsSaleorderFG ClsSaleorderFG = new ClsSaleorderFG();
            this.gvsaleorderdetailsdetails.DataSource = ClsSaleorderFG.LoadNRGP(txtfromdate.Text.Trim(), txttodate.Text.Trim(), Convert.ToString(Session["DEPOTID"]).Trim(), HttpContext.Current.Session["FINYEAR"].ToString().Trim(),"Y");
    ...
    public void loadTransportMode()
    {

        ClsVendor_TPU ClsVendor_TPU = new ClsVendor_TPU();
        this.ddlTransportMode.Items.Clear();
        this.ddlTransportMode.Items.Add(new ListItem("Select Vendor Name", "0"));
        this.ddlTransportMode.AppendDataBoundItems = true;
        this.ddlTransportMode.DataSource = ClsVendor_TPU.BindAllTpuForGateEntry();
    ...
    #region PartyApplicable_SelectedIndexChanged
    protected void PartyApplicable_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (RbApplicable.SelectedValue == "Y")
        {
            this.txtname.Visible = false;
            this.ddlparty.Visible = true;
            this.txtname.Text = "";
            ...
            if (ClsSaleorderMM.NRGPRecordsCheck(ddlProductName.SelectedValue.ToString()) == 0)
            {
               if (this.ddlstorelocation.SelectedValue=="0")
               {
                ...
                if (stockqty >= addqty)
                {
                    ...
                    DataTable dtrecords = ClsSaleorderMM.BindNRGPGridRecords(this.ddlProductName.SelectedValue.ToString(), this.ddlProductName.SelectedItem.Text,
                                                                             ...
                                                                             toalammount, ddlstorelocation.SelectedValue.ToString(),this.ddlstorelocation.SelectedItem.Text.ToString(), "", this.hdn_igstId.Value.ToString()
                    ...
                    this.ddltostorelocation.SelectedValue = "0";
                    this.txtqty.Text = "";
                }
                else
                {
    ...
    //        string flag = clsInvc.BindRegion(this.ddlparty.SelectedValue.Trim(), HttpContext.Current.Session["DEPOTID"].ToString().Trim());

    //        if (string.IsNullOrEmpty(flag))
    //        {
    //            dtTaxCount = clsInvc.ItemWiseTaxCount(Request.QueryString["MENUID"].ToString(), "1", HttpContext.Current.Session["DEPOTID"].ToString().Trim(), this.ddlProductName.SelectedValue.Trim(), this.ddlparty.SelectedValue.Trim(), this.txtsalenrgodate.Text.Trim());
    //        }
    //        else
    //        {
    //            dtTaxCount = clsInvc.ItemWiseTaxCount(Request.QueryString["MENUID"].ToString(), "0", HttpContext.Current.Session["DEPOTID"].ToString().Trim(), this.ddlProductName.SelectedValue.Trim(), this.ddlparty.SelectedValue.Trim(), this.txtsalenrgodate.Text.Trim());
    //        }
    ...
    //        string InvoiceID = Convert.ToString(hdn_saleorderid.Value).Trim();
    //        clsSaleInvoiceMM clsInvc = new clsSaleInvoiceMM();
    ...
    protected void btngrdedit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            //{
                ...
                DataTable dtheader = new DataTable();
                dtheader = ClsSaleorderFG.Editnrgp(hdn_saleorderid.Value, "H", "Y");
                if (dtheader.Rows.Count > 0)
                {
                    ...
                    this.ddltransport.SelectedValue = dtheader.Rows[0]["TRANSPORT"].ToString();
                    this.txtplascedelivery.Text = dtheader.Rows[0]["PLACEOFDELIVERY"].ToString();
                    this.txtgstin.Text = dtheader.Rows[0]["GSTIN"].ToString();
                    if (dtheader.Rows[0]["PARTYTAG"].ToString() == "Y")
                    {
                        this.ddlparty.SelectedValue = dtheader.Rows[0]["PARTYID"].ToString();
                        RbApplicable.Text = dtheader.Rows[0]["PARTYTAG"].ToString();
                        this.ddlparty.Visible = true;
                    }
                    else
                    {
                ...
                ViewState["igstamnt"] = 0;
                ViewState["totalamnt"] = 0;
                dtsaleorderrecord = ClsSaleorderFG.Editnrgp(hdn_saleorderid.Value, "D", "Y");
                gvSaleOrder.DataSource = dtsaleorderrecord;
                gvSaleOrder.DataBind();
                HttpContext.Current.Session["NRGPRECORDS"] = dtsaleorderrecord;
    ...
    protected void btnsave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (dtsaleorderrecord.Rows.Count > 0)
            {
                ...
                salenrgpno = ClsSaleorderFG.InsertRGPDetails(this.txtsalenrgodate.Text.Trim(), this.ddlTransportMode.SelectedValue.ToString(), this.txtname.Text.Trim(),
                                                                     this.lblgatepassno.Text.Trim(), this.txtgatepassdate.Text.Trim(),
                                                                     this.txtremarks.Text, HttpContext.Current.Session["UserID"].ToString().Trim(),
                                                                     HttpContext.Current.Session["FINYEAR"].ToString().Trim(), xml, this.hdn_saleorderid.Value.ToString(),
                                                                     TotalCase, TotalPCS, Convert.ToString(HttpContext.Current.Session["DEPOTID"]), this.ddlSuppliedItem.SelectedItem.Text.Trim(),
                                                                     this.txtlrgrno.Text.Trim(), this.txtVehicleno.Text.Trim(), this.ddltransport.SelectedValue.Trim(), this.txtplascedelivery.Text.Trim(),
                                                                     this.txtgstin.Text.Trim(),this.ddlparty.SelectedValue.Trim(), this.RbApplicable.SelectedValue.Trim());
    ...
    protected void btnNRGPPrint_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            upath = "frmRptInvoicePrint_FAC.aspx?NRGPIvoicveid=" + hdn_saleorderid.Value.Trim() + "&&TAG=RGP&&MenuId=144";
    ...
    public void LoadStoreLocation()
    {
        ...
        if (dtlocation.Rows.Count > 0)
        {
            ...
            this.ddltostorelocation.Items.Clear();
            this.ddltostorelocation.Items.Add(new ListItem("Select Store Location", "0"));
            this.ddltostorelocation.AppendDataBoundItems = true;
            this.ddltostorelocation.DataValueField = "ID";
            this.ddltostorelocation.DataTextField = "NAME";
            this.ddltostorelocation.DataSource = dtlocation;
            this.ddltostorelocation.DataBind();            
        }
    }
    #endregion
