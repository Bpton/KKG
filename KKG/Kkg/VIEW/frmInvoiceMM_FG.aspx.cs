...
public partial class VIEW_frmInvoiceMM_FG : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            {
                string TAXINVOICEID = Convert.ToString(Request.QueryString["VCHID"]);
                this.lblStockRcvdNo.Text = "Invoice No.";
                this.Label2.Text = "Invoice Date";
                this.Label10.Text = "GATEPASS NO";
                divbtnapprove.Visible = false;
                ItemLedger_TAXINVOICE(TAXINVOICEID);
            }
            else
            {
                ...
                else
                {
                    ...
                    this.CalendarExtenderInvoiceDate.Enabled = false;
                }
                #endregion
                ...
                if (Convert.ToString(Request.QueryString["CHALLAN"]).Trim() == "FALSE")
                {
                    ...
                    this.Label2.Text = "Invoice Date";
                    this.Label10.Text = "GATEPASS NO";
                    this.txtGetpass.Attributes.Add("placeholder", "INVOICE NO");
    ...
    protected void BindTransporter(string DepotID)
    {
        ...
        clsSaleInvoiceFG clsInvc = new clsSaleInvoiceFG();
    ...
    protected void BindInvoiceCustomer()
    {
        ...
        DataTable dtDepot = new DataTable();
        clsSaleInvoiceFG clsSaleInvoiceFG = new clsSaleInvoiceFG();
        dtCustomer = clsSaleInvoiceFG.BindCustomer(ViewState["BSID"].ToString().Trim(), "", this.ddlDepot.SelectedValue.Trim());
        ...
        if (this.hdnDespatchID.Value == "")
        {
            if (dtCustomer.Rows.Count > 0)
            {
                ...
                if (dtCustomer.Rows.Count == 1)
                {
                    ...
                    //this.BindWayBill();
                    this.BindSaleOrder(this.ddlDistributor.SelectedValue.Trim());
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        this.ddlSaleOrder.Enabled = true;
    ...
    protected void ddlProduct_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        {
            this.LoadProductWisePacksize(this.ddlProduct.SelectedValue.ToString().Trim());
            //if (this.ddlPacksize.SelectedValue != "0")
            //{
            //    this.BindOrderQty(this.ddlDistributor.SelectedValue.Trim(), this.ddlSaleOrder.SelectedValue.Trim(), this.ddlProduct.SelectedValue.Trim(), Convert.ToString(Session["DEPOTID"]));
            //    this.LoadBatchDetails(this.ddlProduct.SelectedValue.Trim(), this.ddlPacksize.SelectedValue.Trim(), Convert.ToString(Session["DEPOTID"]), "0");
    ...
    {
        clsSaleInvoiceFG Inv = new clsSaleInvoiceFG();
    ...
    #region Bind Waybill
    protected void BindWayBill()
    {
        clsSaleInvoiceFG clsInvc = new clsSaleInvoiceFG();
        DataTable dtWaybillNo = clsInvc.BindWaybill(this.ddlDistributor.SelectedValue.Trim());
        if (dtWaybillNo.Rows.Count > 0)
        {
            if (dtWaybillNo.Rows.Count > 1)
            {
                this.ddlWaybill.Items.Clear();
                this.ddlWaybill.Items.Add(new ListItem("Select E-Waybill", "0"));
            ...
            else if (dtWaybillNo.Rows.Count == 1)
            {
                this.ddlWaybill.Items.Clear();
                this.ddlWaybill.Items.Add(new ListItem("Select E-Waybill", "0"));
                this.ddlWaybill.AppendDataBoundItems = true;
                ...
                this.ddlWaybill.DataTextField = "WAYBILLNO";
                this.ddlWaybill.DataBind();
                this.ddlWaybill.SelectedValue = Convert.ToString(dtWaybillNo.Rows[0]["WAYBILLNO"]);
            }
        }
        else
        {
    ...
    #region Bind SaleOrder
    public void BindSaleOrder(string CustomerID)
    {
        clsSaleInvoiceFG clsInvc = new clsSaleInvoiceFG();
        //DataTable dtSaleOrderNo = clsInvc.BindSaleOrder(CustomerID);
        DataSet ds = new DataSet();
        ds = clsInvc.BindSaleOrder(CustomerID, HttpContext.Current.Session["FINYEAR"].ToString());

        if (ds.Tables[0].Rows.Count > 0)
        {
            this.ddlSaleOrder.Items.Clear();
            this.ddlSaleOrder.Items.Add(new ListItem("Select Order No", "0"));
            this.ddlSaleOrder.AppendDataBoundItems = true;
            this.ddlSaleOrder.DataSource = ds.Tables[0];
            this.ddlSaleOrder.DataValueField = "SALEORDERID";
            this.ddlSaleOrder.DataTextField = "SALEORDERNO";
            this.ddlSaleOrder.DataBind();
            /*if (dtSaleOrderNo.Rows.Count == 1)
            {
                this.ddlSaleOrder.SelectedValue = Convert.ToString(dtSaleOrderNo.Rows[0]["SALEORDERID"]);
                this.LoadOrderProduct();
            }*/

            txtShippingAddress.Text = ds.Tables[1].Rows[0]["Address"].ToString();
        }
        else
        {
            this.ddlSaleOrder.Items.Clear();
        ...
        if (this.ddlProduct.SelectedValue != "0" && this.ddlPacksize.SelectedValue != "0")
        {
            if (this.ddlbatchno.SelectedValue != "0")
            {
                if (this.hdnGroupID.Value != "0")
                {
                    ...
                    this.txtAssementPercentage.Text = ddlbatchno.SelectedItem.Text.Substring(26, 14).Trim();
    ...
    {
        clsSaleInvoiceFG ClsInv = new clsSaleInvoiceFG();
    ...
    #region ddlSaleOrder_SelectedIndexChanged
    protected void ddlSaleOrder_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (ddlSaleOrder.SelectedValue != "0")
        {
            try
            {
                this.LoadOrderProduct();
                ScriptManager.RegisterClientScriptBlock(this, typeof(Page), "FocusOnState", "document.getElementById('" + ddlTransporter.ClientID + "').focus(); ", true);
            }
            catch (Exception ex)
            {
                string message = "alert('" + ex.Message.Replace("'", "") + "')";
    ...
    protected void LoadOrderProduct()
    {
        ...
        dtOrderDetails = clsInvc.BindOrderProduct(this.ddlSaleOrder.SelectedValue.Trim());
    ...
    {
        String[] batchno = ddlbatchno.SelectedValue.Trim().ToString().Split('|');
        ...
        decimal orderedqty = Convert.ToDecimal(this.txtOrderQty.Text.Trim());
        decimal stockqty = Convert.ToDecimal(this.txtStockQty.Text.Trim());
        decimal RemainingQty = Convert.ToDecimal(this.txtRemainingQty.Text.Trim());
        ...
        if (deliveredqty > 0)
        {
            ...
            {
                MessageBox1.ShowInfo("<b>Delivered Qty should not be greater than Ordered Qty</b>", 40, 500);
                return;
            }
            ...
            int numberOfRecords = dtInvoice.Select("SALEORDERID = '" + this.ddlSaleOrder.SelectedValue + "' AND PRODUCTID = '" + this.ddlProduct.SelectedValue + "'AND BATCHNO='" + BATCHNO + "'").Length;
            if (this.chkFree.Checked == false)
            {
                ...
                else
                {
                    ...
                    if (ddlPacksize.SelectedValue == "B9F29D12-DE94-40F1-A668-C79BF1BF4425")
                    {
                        CaseQty = clsInvc.ConvertPcsToCase(ddlProduct.SelectedValue.ToString(), "B9F29D12-DE94-40F1-A668-C79BF1BF4425", "1970C78A-D062-4FE9-85C2-3E12490463AF"/*ddlPacksize.SelectedValue.ToString()*/, deliveredqty);
                    }
                    else
                    {
                        CaseQty = deliveredqty;
                        deliveredqty = clsInvc.ConvertPcsToCase(ddlProduct.SelectedValue.ToString(), "1970C78A-D062-4FE9-85C2-3E12490463AF", "B9F29D12-DE94-40F1-A668-C79BF1BF4425", deliveredqty);
                    }

                    this.FillPrimaryGridView(deliveredqty, this.ddlSaleOrder.SelectedValue.Trim(),
                                             ...
                                             Convert.ToString(this.ddlProduct.SelectedItem).Trim(), BATCHNO, this.ddlbatchno.SelectedItem.Text.Trim(),
                                             Convert.ToDecimal(this.txtBaseCost.Text.Trim()), MRP, AssementPercentage,
                                             TotalAmount, TotalTax, GrossTotal, GrossFinal, TotalMRP, RateDisc, NSR, CaseQty);
                }
            }
        }
        else
    ...
    #region CalculateFreeItem
    private void CalculateFreeItem(ref DataTable dtQtyScheme, ref DataTable dtFreeProductDetails, string PRIMARYBATCHNO)
    {
        clsSaleInvoiceFG clsInvc = new clsSaleInvoiceFG();
        bool flag = false;
        ...
        this.ddlFreeProduct.Items.Clear();
        this.ddlFreeProduct.Items.Add(new ListItem("Select Product", "0"));

        for (int freeCounter = 0; freeCounter < dtQtyScheme.Rows.Count; freeCounter++)
        {
            string Scheme_Product = dtQtyScheme.Rows[freeCounter]["SCHEME_PRODUCT_ID"].ToString();
            string FreeProduct = dtQtyScheme.Rows[freeCounter]["PRODUCTID"].ToString();
            decimal FreeQty = Convert.ToDecimal(dtQtyScheme.Rows[freeCounter]["SCHEME_QTY"].ToString());

            if (Session["INVOICEQTYSCHEMEDETAILS"] != null)
            {
                dtFreeProductDetails = (DataTable)Session["INVOICEQTYSCHEMEDETAILS"];
                DataRow[] rowsFreeProductDetails = dtFreeProductDetails.Select("PRODUCTID='" + FreeProduct + "'");

                decimal difference = 0;
                decimal TotalFreeQty = 0;

                TotalFreeQty = CalculateTotalFreeQty(dtFreeProductDetails, FreeProduct, Scheme_Product);
                if (TotalFreeQty == 0)
                {
                    difference = FreeQty - TotalFreeQty;
                }
                ...
                if (difference > 0)
                {
                    ...
                    if (Convert.ToString(dtQtyScheme.Rows[freeCounter]["BATCHNO"]) == "-NA")
                    {
                        flag = true;
                        this.ddlFreeProduct.Items.Add(new ListItem(Convert.ToString(dtQtyScheme.Rows[freeCounter]["PRODUCTNAME"]) + "(" + Convert.ToString(difference) + ")", Convert.ToString(dtQtyScheme.Rows[freeCounter]["PRODUCTID"])));
                    }
                    else
                    {
                        dtQtyScheme.Rows[freeCounter]["SCHEME_QTY"] = Convert.ToString(difference);
                        dtQtyScheme.Rows[freeCounter]["SCHEME_PRODUCT_ID"] = Convert.ToString(this.ddlProduct.SelectedValue.Trim());
                        dtQtyScheme.Rows[freeCounter]["SCHEME_PRODUCT_NAME"] = Convert.ToString(this.ddlProduct.SelectedItem).Substring(0, this.ddlProduct.SelectedItem.ToString().IndexOf("~")).Trim();
                        dtQtyScheme.Rows[freeCounter]["SCHEME_PRODUCT_BATCHNO"] = PRIMARYBATCHNO.Trim();
                        decimal Amount = clsInvc.CalculateAmountInPcs(Convert.ToString(dtQtyScheme.Rows[freeCounter]["PRODUCTID"]).Trim(), Convert.ToString(dtQtyScheme.Rows[freeCounter]["PACKSIZEID"]).Trim(),
                                                                    difference, Convert.ToDecimal(Convert.ToString(dtQtyScheme.Rows[freeCounter]["NSR"]).Trim()), 0, 0);
                        dtQtyScheme.Rows[freeCounter]["Amount"] = Convert.ToString(String.Format("{0:0.00}", Amount)).Trim();
        ...
        if (flag == true)
        {
            if (this.ddlFreeProduct.Items.Count > 0)
            {
                if (this.ddlFreeProduct.Items.Count == 2)
                {
                    this.ddlFreeProduct.SelectedIndex = 1;
    ...
    protected void FillPrimaryGridView(decimal deliveredqty, string SALEORDERID, string DDLPACKSIZEID, string DDLPACKSIZENAME,
                                        string ORDERDATE, string PRODUCTID, string PRODUCTNAME, string BATCHID, string ddlbatchno,
                                        ...
                                        decimal RateDisc, decimal NSR, decimal CaseQty)
    {
        clsSaleInvoiceFG clsInvc = new clsSaleInvoiceFG();
        DataTable dtInvoice = (DataTable)Session["SALEINVOICEDETAILS"];
        ...
        //if (DDLPACKSIZEID == "1970C78A-D062-4FE9-85C2-3E12490463AF")
        //{
        //    Obj = clsstocktransfer.DispatchAmtPcs(PRODUCTID, DDLPACKSIZEID, CaseQty, Convert.ToDecimal(MRP), Convert.ToDecimal(ASSESMENTPERCENTAGE));
        //}
        //else
        //{
        ...
        dr["QTY"] = Convert.ToString(CaseQty);
        dr["QTYPCS"] = Convert.ToString(deliveredqty);
        dr["BATCHNO"] = Convert.ToString(BATCHID);
        ...
        dr["MFDATE"] = Convert.ToString(ddlbatchno.Substring(40, 14).Trim());
        dr["EXPRDATE"] = Convert.ToString(ddlbatchno.Substring(54, 14).Trim());
        ...
        for (int k = 0; k < dtTaxCountDataAddition.Rows.Count; k++)
        {
            ...
            CreateTaxDatatable(SALEORDERID, Convert.ToString(PRODUCTID), Convert.ToString(BATCHID), Convert.ToString(PRODUCTID), Convert.ToString(BATCHID), dtTaxCountDataAddition.Rows[k]["TAXID"].ToString(), Convert.ToString(Taxpercent),
            ...
            if (ObjDate == true)
            {
                ...
                else
                {
                    ...
                    InvoiceNo = clsInvc.InsertInvoiceDetails(this.txtInvoiceDate.Text.Trim(), this.ddlDistributor.SelectedValue.Trim(),
                                                             Convert.ToString(this.ddlDistributor.SelectedItem).Trim(), this.ddlWaybill.SelectedValue.Trim(),
                                                             this.ddlTransporter.SelectedValue, this.txtVehicle.Text.Trim(), this.ddlDepot.SelectedValue,
                                                             Convert.ToString(this.ddlDepot.SelectedItem).Trim(), ViewState["BSID"].ToString().Trim(),
                                                             ...
                                                             Adjustment, Roundoff, Disc, strTermsID,
                                                             xml, xmlTax,
    ...
    protected void btngrdedit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (Verify == "Y")
            {
                ...
                this.txtwaybilno.Visible = true;
                this.txtwaybilldate.Visible = true;
                this.imgPopupwaybilldate.Visible = true;
                this.lblwaybildate.Visible = true;
                this.txtwaybilno.Visible = true;
                this.lblwaybillno.Visible = true;
            }
            else
            {
            ...
            this.ddlSaleOrder.Enabled = false;
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                this.txtShippingAddress.Text = Convert.ToString(ds.Tables[0].Rows[0]["DELIVERYADDRESS"]).Trim();
                ...
                dtEditOrderDetails = clsInvc.BindOrderProduct(this.ddlSaleOrder.SelectedValue.Trim());
                    ...
                    for (int k = 0; k < dtTaxCountDataAddition.Rows.Count; k++)
                    {
                        ...
                        switch (Convert.ToString(dtTaxCountDataAddition.Rows[k]["RELATEDTO"]))
                        {
                            case "1":
                                ...
                                //drEditInvoice["" + Convert.ToString(dtTaxCountDataAddition.Rows[k]["NAME"]) + ""] = Convert.ToString(String.Format("{0:0.00}", AfterRebateAmount * Taxpercent / 100));
                                //Totaltax += Convert.ToDecimal(String.Format("{0:0.00}", AfterRebateAmount * Taxpercent / 100));
                                //break;
    ...
    protected void btnWaybillUpdate_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (flag == 1)
            {
                ...
                this.grdDespatchHeader.DataSource = clsInvc.BindFG_SaleInvoice(this.txtFromDate.Text.Trim(), this.txtToDate.Text.Trim(),
    ...
    protected void ClearControls()
    {
        ...
        this.ddlSaleOrder.SelectedValue = "0";
        this.txtOrderDate.Text = "";
    ...
    public DataTable CreateDataTable()
    {
        ...
        if (hdnDespatchID.Value == "")
        {
            ...
            if (Convert.ToString(Request.QueryString["Name"]) != "Itemledger")  /*  REDIRECT Item Ledger to tkis page   */
            {
            ...
            else
            {
                ...
                DataSet ds = new DataSet();
                string InvoiceID = Convert.ToString(Request.QueryString["VCHID"]);
                clsSaleInvoiceFG clsInvc1 = new clsSaleInvoiceFG();
                ds = clsInvc1.EditInvoiceDetails(InvoiceID);
    ...
    {
        clsSaleInvoiceFG clsInvoice = new clsSaleInvoiceFG();
    ...
    protected void TotalCaseCalculation()
    {
        ...
        foreach (GridViewRow row in grdAddDespatch.Rows)
        {
            TotalBillCase += Convert.ToDecimal(row.Cells[13].Text.Trim());
        ...
        {
            this.txtTotCase.Text = Convert.ToString(String.Format("{0:0}", TotalBillCase)).Trim() + '.' + Convert.ToString(String.Format("{0:0}", TotalPCS)).Trim();
        }
    ...
    public void LoadInvoice()
    {
        ...
        {
            clsSaleInvoiceFG clsInvc = new clsSaleInvoiceFG();
            this.grdDespatchHeader.DataSource = clsInvc.BindFG_SaleInvoice(txtFromDate.Text.Trim(),
                                                                    txtToDate.Text.Trim(),
                                                                    HttpContext.Current.Session["FINYEAR"].ToString().Trim(),
                                                                    ViewState["BSID"].ToString().Trim(),
                                                                    HttpContext.Current.Session["DEPOTID"].ToString().Trim(),
    ...
    protected void btngrdUpdateForm_Click(object sender, EventArgs e)
    {
        try
        {
            clsSaleInvoiceFG clsSaleInvoiceMM = new clsSaleInvoiceFG();
            string despatchID = Convert.ToString(hdnDespatchID.Value).Trim();
    ...
    public void LoadBatchDetails(string ProductID, string PacksizeID, string DepotID, string BatchNo)
    {
        ...
        {
            DataTable dtBatch = new DataTable();
            List<USP_BATCHWISE_FACTORY_STOCK_Result> ObjStock = new List<USP_BATCHWISE_FACTORY_STOCK_Result>();
            ObjStock = clsstocktransfer.BindBatchDetails_edmx(DepotID, ProductID, PacksizeID, BatchNo);
            dtBatch = ObjCommonFunction.ToDataTable(ObjStock);
    ...
    #region REDIRECT Item Ledger to this
    public void ItemLedger_TAXINVOICE(string TAXINVOICEID)

    {
        ...
        this.btnaddhide.Style["display"] = "none";
        this.trproductadd.Visible = false;
        string saleInvoiceID = Convert.ToString(TAXINVOICEID).Trim();
        Verify = clsInvc.CheckVerify(saleInvoiceID);
        if (Verify == "Y")
        {
            btnsubmitdiv.Style["display"] = "none";
        ...
        if (ds.Tables[6].Rows.Count > 0)
        {
            if ((DataTable)Session["INVOICETAXCOMPONENTDETAILS"] == null)
            {
        ...
        if (ds.Tables[1].Rows.Count > 0)
        {
            if ((DataTable)Session["SALEINVOICEDETAILS"] == null)
            {
                CreateDataTable();
            }
            dtInvoiceEdit = (DataTable)Session["SALEINVOICEDETAILS"];
            for (int i = 0; i < ds.Tables[1].Rows.Count; i++)
            {
                ...
                #region Loop For Adding Itemwise Tax Component
                DataTable dtTaxCountDataAddition = (DataTable)Session["dtInvoiceTaxCount"];
                if ((DataTable)Session["dtInvoiceTaxCount"] == null)
                {
                    CreateDataTable();
                }
