...
namespace BAL
{
    public class ClsTransporterBill
    {
        ...
        public DataTable BindTransporterByStock(string tag)
        {
            ...
            {
                sql = "SELECT DISTINCT A.ID,A.NAME from M_TPU_TRANSPORTER A INNER JOIN T_SALERETURN_HEADER B ON  A.ID=B.TRANSPORTERID";
            }
            else
            {
        ...
        //{
        //    string sql = " SELECT TRANSPOTERID,TRANSPOTERNAME FROM M_TRANSPOTER_DEPOT_MAP WHERE DEPOTID='" + DEPOTID + "' ORDER BY TRANSPOTERNAME ";
        //    DataTable dt = new DataTable();
        //    dt = db.GetData(sql);
        //    return dt;
        //}
        ...
        public DataTable BindCheakerTransporterbill(string FromDate, string ToDate)
        {
            ...
            string sql = string.Empty;
            sql = "EXEC USP_BIND_TRANSPORTERBILL '8','','" + FromDate + "','" + ToDate + "'";
            dt = db.GetData(sql);
            return dt;
        }
        ...
        public string GetstatusChecking(string TRANSPORTERBILLID)
        {
            ...
            string Sql = " SP_TRANSPORTERBILL_DELETE_CHECKING '" + TRANSPORTERBILLID + "' ";
            return ((string)db.GetSingleValue(Sql));
        }
        ...
        public DataTable BindLRNoByStock(string tag, string transporterid, string DEPOTID)
        {
            ...
            else if (tag == "STKINV")
            {
                //sql = "SELECT ISNULL(LRGRNO,' ')+' '+SALEINVOICEPREFIX+'/'+SALEINVOICENO+'/'+SALEINVOICESUFFIX AS LRGRNO,LRGRNO AS LRID from T_SALEINVOICE_HEADER WHERE TRANSPORTERID='" + transporterid + "' AND LRGRNO <>'' "+
                //    " and  ISNULL(LRGRNO,' ')+' '+SALEINVOICEPREFIX+'/'+SALEINVOICENO+'/'+SALEINVOICESUFFIX NOT IN (SELECT * from dbo.fnSplit('" + LRGR + "',','))";
                sql = "SELECT ISNULL(LRGRNO,' ') AS LRGRNO,SALERETURNID AS LRID from T_SALERETURN_HEADER WHERE TRANSPORTERID='" + transporterid + "' /* AND LRGRNO <>'' " +
                    "   and  ISNULL(LRGRNO,' ') NOT IN (SELECT * from dbo.fnSplit('" + LRGR + "',',')) */ and DEPOTID='" + DEPOTID + "'";
            }
            // DataTable dt = new DataTable();
            dt = db.GetData(sql);
        ...
        public DataTable BindLRNoByTransporterGroup(string tag, string transporterid)
        {
            ...
            else if (tag == "STKSR")
            {
                //sql = "SELECT ISNULL(LRGRNO,' ')+' '+SALEINVOICEPREFIX+'/'+SALEINVOICENO+'/'+SALEINVOICESUFFIX AS LRGRNO,LRGRNO AS LRID from T_SALEINVOICE_HEADER WHERE TRANSPORTERID='" + transporterid + "' AND LRGRNO <>'' "+
                //    " and  ISNULL(LRGRNO,' ')+' '+SALEINVOICEPREFIX+'/'+SALEINVOICENO+'/'+SALEINVOICESUFFIX NOT IN (SELECT * from dbo.fnSplit('" + LRGR + "',','))";
                sql = "SELECT ISNULL(LRGRNO,' ') AS LRGRNO,SALERETURNID AS LRID from T_SALERETURN_HEADER WHERE TRANSPORTERID IN (SELECT * from dbo.fnSplit('" + Transporter + "',',')) AND LRGRNO <>'' " +
                    " and  ISNULL(LRGRNO,' ') NOT IN (SELECT * from dbo.fnSplit('" + LRGR + "',','))";
            }
            // DataTable dt = new DataTable();
            dt = db.GetData(sql);
        ...
        public DataTable Bindlrnowithrespectinvono(string tag, string invno)
        {
            ...
            {
                sql = "SELECT SALERETURNID as LRID,ISNULL(LRGRNO,' ') AS LRGRNO from T_SALERETURN_HEADER" +
                   " WHERE SALERETURNID='" + invno + "'";
        ...
        public DataTable BindAllInvoiceno(string tag, string transporterid, string DEPOTID)
        {
            ...
            {
                sql = " SELECT SALERETURNID as INVID,(SALERETURNPREFIX+'/'+SALERETURNNO+' /'+SALERETURNSUFFIX) AS INVNO from T_SALERETURN_HEADER" +
                      " WHERE TRANSPORTERID='" + transporterid + "' and DEPOTID='" + DEPOTID + "'";
        ...
        public DataTable BindAllInvoicenoNEW(string tag, string transporterid, string DEPOTID)/*subho*/
        {
            ...
            else if (tag == "STKSR")
            {
                ...
                sql = " SELECT SALERETURNID as INVID,GETPASSNO+'-('+SALERETURNPREFIX+'/'+SALERETURNNO+' /'+SALERETURNSUFFIX+')' AS INVNO from T_SALERETURN_HEADER " +
                      " WHERE SALERETURNID NOT IN (SELECT STOCKBILLID FROM T_TRANSPORTER_BILL_DETAILS) AND DEPOTID='" + DEPOTID + "'";
        ...
        public DataTable BindAllInvoicenoNEWV2(string tag, string transporterid, string DEPOTID, string transporterbillid)/*subho*/
        {
            ...
            string sqlTT = " EXEC [USP_BIND_INVOICENO_ERP] '" + tag + "','" + DEPOTID + "','" + transporterbillid + "','" + finyear + "'";
            DataTable dt = db.GetData(sqlTT);
            return dt;
            /***************************************************************************************************/
        ...
        public DataTable BindTransGroupInvoiceno(string tag, string transporterid)
        {
            ...
            {
                sql = "SELECT SALERETURNID as INVID,ISNULL(GETPASSNO,'')+'-('+SALERETURNPREFIX+'/'+SALERETURNNO+' /'+SALERETURNSUFFIX+')' AS INVNO from T_SALERETURN_HEADER" +
                   " WHERE TRANSPORTERID IN (SELECT * from dbo.fnSplit('" + Transporter + "',','))";
        ...
        public DataTable BINDINVOICENOV2(string tag, string id)
        {
            ...
            {
                SQL = "SELECT SALERETURNID as INVID,ISNULL(GETPASSNO,'')+'-('+SALERETURNPREFIX+'/'+SALERETURNNO+' /'+SALERETURNSUFFIX+')' AS INVNO " +
                    " FROM T_SALERETURN_HEADER WHERE SALERETURNID='" + id + "'";
            }
        ...
        public decimal TDSPercentage(string transporterid,decimal Value)
        {
            ...
            //sql = " SELECT CASE WHEN ISNULL(ISTDSDECLARE,'N')='N' THEN (SELECT PERCENTAGE FROM M_TAX WHERE ID IN(SELECT TDS_ACC_LEADGER FROM P_APPMASTER)) " +
            //      " ELSE '0' END AS PERCENTAGE FROM M_TPU_TRANSPORTER  WHERE ID='" + transporterid + "' ";


            //SELECT TDS_ACC_LEADGER FROM P_APPMASTER
        ...
        public string BindLRGRbytransporter(string LRGRNO,string Invoicetype)
        {
            ...
            if (Invoicetype == "STKSR")
            {
                sql = "SELECT TRANSPORTERID FROM T_SALERETURN_HEADER WHERE SALERETURNID='" + LRGRNO + "'";
            }
        ...
        public int updatetransporter(string LRGRNO, string Invoicetype,string transporterid)
        {
            ...
            if (Invoicetype == "STKSR")
            {
                sql = "UPDATE T_SALERETURN_HEADER SET TRANSPORTERID ='" + transporterid + "' where SALERETURNID='" + LRGRNO + "'";
            }
        ...
        public DataTable BindTransporterbyInvoice(string TAG, string InvoiceID)
        {
            ...
            if (TAG == "STKINV")
            {
                sql = " SELECT TRANSPORTERID,NAME AS TRANSPORTERNAME,LRGRNO,DISTRIBUTORNAME," +
                      ...
                      " UNION "+
                      " SELECT TRANSPORTERID,NAME AS TRANSPORTERNAME,LRGRNO,DISTRIBUTORNAME," +
                      " CASE WHEN CONVERT(VARCHAR(10), CAST(LRGRDATE AS DATE),103)='01/01/1900' THEN CONVERT(VARCHAR(10),CAST(GETDATE() AS DATE),103)" +
                      " ELSE " +
                      " CONVERT(VARCHAR(10),CAST(LRGRDATE AS DATE),103) END AS LRGRDATE" +
            ...
            {
                sql = " SELECT TRANSPORTERID,NAME AS TRANSPORTERNAME,LRGRNO,TODEPOTNAME DISTRIBUTORNAME," +
                      " CASE WHEN CONVERT(VARCHAR(10), CAST(LRGRDATE AS DATE),103)='01/01/1900' THEN CONVERT(VARCHAR(10),CAST(GETDATE() AS DATE),103)" +
                      " ELSE " +
                      " CONVERT(VARCHAR(10),CAST(LRGRDATE AS DATE),103) END AS LRGRDATE" +
            ...
            {
                sql = " SELECT TRANSPORTERID,NAME AS TRANSPORTERNAME,LRGRNO,RECEIVEDDEPOTNAME DISTRIBUTORNAME," +
                      " CASE WHEN CONVERT(VARCHAR(10), CAST(LRGRDATE AS DATE),103)='01/01/1900' THEN CONVERT(VARCHAR(10),CAST(GETDATE() AS DATE),103)" +
                      " ELSE " +
                      " CONVERT(VARCHAR(10),CAST(LRGRDATE AS DATE),103) END AS LRGRDATE" +
            ...
            {
                sql = " SELECT TRANSPORTERID,NAME AS TRANSPORTERNAME,LRGRNO,TPUNAME DISTRIBUTORNAME, " +
                      " CASE WHEN CONVERT(VARCHAR(10), CAST(LRGRDATE AS DATE),103)='01/01/1900' THEN CONVERT(VARCHAR(10),CAST(GETDATE() AS DATE),103)" +
                      " ELSE " +
                      " CONVERT(VARCHAR(10),CAST(LRGRDATE AS DATE),103) END AS LRGRDATE" +
        ...
        public string SaveTransporterbillV2(string ID, string Mode, string TransporterID, string TransporterName, string billdate, string billtype, string Remarks,
                                      string UserID, string finyear, string xml, string MenuId, decimal TOTALNETAMOUNT,
                                      decimal TOTALTDS, decimal TOTALGROSSWEIGHT, string depotid, string depotname, char IsTransferToHO, decimal TOTALBILLAMOUNT,
                                      decimal TOTALTDSDEDUCTABLE, decimal sumcgst, decimal sumsgst, decimal sumigst, decimal sumugst, string BILLINGFROMSTATEID,
                                      string Reversecharge, string GNGNO, string TDSAPPLICABLE, decimal TDSPECENTAGE, string TDSID, string VIRTUALdepotid, string EXPORTTAG, string ReasonID, string GSTReasonID)
        {
            string transporterbillno = string.Empty;
            string sql = string.Empty;

            sql = "EXEC SP_TRANSPORTERBILL_INSERT_UPDATE_GST_V2 '" + ID + "','" + Mode + "','" + TransporterID + "','" + TransporterName + "','" + billdate + "','" + billtype + "' ," +
                                                         " '" + Remarks + "','" + UserID + "','" + finyear + "','" + xml + "','" + MenuId + "'," + TOTALNETAMOUNT + "," +
                                                        " " + TOTALTDS + "," + TOTALGROSSWEIGHT + ",'" + depotid + "','" + depotname + "'," + IsTransferToHO + "," +
        ...
        public DataTable BindTransporterbillall(string depotid, string fromData, string ToDate)
        {
            DataTable dt = new DataTable();

            string sql = string.Empty;
            sql = "EXEC USP_BIND_TRANSPORTERBILL_FORALLUSER '" + depotid + "','" + fromData + "','" + ToDate + "'";
        ...
        {
            string qury = " SELECT ISNULL(OFFLINE,'Y') AS OFFLINE FROM M_BRANCH WHERE BRID='" + BRID + "'";
            string OFFLINE = (string)db.GetSingleValue(qury);
            return OFFLINE;
        }

        public string GetOfflineStatusDepot()
        {
            string qury = " SELECT OFFLINE FROM P_APPMASTER";
            string OFFLINE = (string)db.GetSingleValue(qury);
            return OFFLINE;
        }


        public string GetOfflineStatusUser(string USERID)
        {
            string qury = " SELECT ISNULL(OFFLINE,'Y') FROM M_BRANCH A INNER JOIN" +
                          " M_TPU_USER_MAPPING B ON B.TPUID=A.BRID" +
                          " WHERE B.USERID='" + USERID + "' AND B.TPUID=(SELECT KOLKATADEPOTID FROM P_APPMASTER)";
            string OFFLINE = (string)db.GetSingleValue(qury);
            return OFFLINE;
        }
    }
