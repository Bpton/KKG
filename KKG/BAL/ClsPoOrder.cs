...
namespace BAL
{
    public class ClsPoOrder
    {
        ...
        public DataTable BindPO(string fromdate, string todate, string FinYear)
        {
            string sql = " select A.POID,PONO,CONVERT(VARCHAR(10),CAST(PODATE AS DATE),103) AS PODATE,TPUNAME,ISNULL(TOTALCASEPACK,0) AS TOTALCASEPACK,ORDERTYPEID,ORDERTYPENAME,COUNT(DISTINCT PRODUCTID) AS TOTALPRODUCT," +
                         " CASE WHEN ISCLOSED='Y' THEN 'CLOSED' ELSE 'OPEN' END AS STATUS" +
                         " from [T_TPU_POHEADER] A WITH (NOLOCK) " +
                         " INNER JOIN T_TPU_PODETAILS B WITH (NOLOCK) ON A.POID=B.POID where CONVERT(date,PODATE,103) between dbo.Convert_To_ISO('" + fromdate + "') and dbo.Convert_To_ISO('" + todate + "') AND FINYEAR='" + FinYear + "' AND REFERENCEPOID=''" +
                         " GROUP BY A.POID,PONO,PODATE,TPUNAME,TOTALCASEPACK,ORDERTYPEID,ORDERTYPENAME,ISCLOSED ORDER BY dbo.ExtractDate_To_ISO(PODATE) DESC";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        public DataTable BindPO_kkg(string fromdate, string todate, string FinYear, string label, string type) /*NEW METHOD FOR KKG PO BY P.BASU ON 17032020*/
        {
            string sql = " USP_SEARCH_BIND_PO '" + fromdate + "','" + todate + "','" + FinYear + "','" + label + "','" + type + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        public DataTable BindPOAmendment(string fromdate, string todate, string FinYear)
        {
            string sql = "select B.PONO,A.PONO AS REFERENCEPONO,CONVERT(VARCHAR(10),CAST(B.PODATE AS DATE),103) AS PODATE,B.TPUNAME from [T_TPU_POHEADER] A WITH (NOLOCK),[T_TPU_POHEADER] B WITH (NOLOCK)" +
                         "WHERE A.POID=B.REFERENCEPOID AND CONVERT(date,B.PODATE,103) between dbo.Convert_To_ISO('" + fromdate + "') and dbo.Convert_To_ISO('" + todate + "') AND B.FINYEAR='" + FinYear + "' AND B.ISCLOSED='N' order by dbo.ExtractDate_To_ISO(B.PODATE) desc";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        ...
        {
            string sql = "select VENDORID, VENDORNAME  from M_TPU_VENDOR where suplieditemid=1 and tag='T'  order by VENDORNAME";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }

        public DataTable BindReqTPU()
        {
            string sql = " EXEC USP_GET_REQ_TPU";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }

        public DataTable BindRequirement(string FromDate, string ToDate, string Finyear)
        {
            string sql = " EXEC USP_GET_PROD_REQ '" + FromDate + "','" + ToDate + "','" + Finyear + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }

        public DataTable BindBindiTPU()
        {
            string sql = "select VENDORID, VENDORNAME  from M_TPU_VENDOR where suplieditemid=1 and tag='T' AND STATEID = '28'  order by VENDORNAME";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        ...
        {
            string sql = " SELECT OrderTYPEID AS BSID,ORDERTYPENAME AS BSNAME FROM M_ORDERTYPE ORDER BY ORDERTYPENAME DESC";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        ...
        {
            string sql = "SELECT CASEPACKSIZEID AS PACKSIZEID,CASEPACKSIZENAME AS PACKSIZENAME FROM P_APPMASTER" +
                         " UNION " +
                         " SELECT '71B973E8-28E3-4F3A-A86E-9475AF2D14EE' AS PACKSIZEID ,'BOX' AS PACKSIZENAME FROM M_PACKINGSIZE  " +
                         " UNION " +
                         "SELECT PACKSIZE AS PACKSIZEID,PACKSIZENAME FROM P_APPMASTER";
        ...
        public DataTable BindPacksizeProductwise(string productid)
        {
            string sql = "usp_packsize_with_converttionqty_kkg '" + productid + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        ...
        {
            string sql = "SELECT SUBSTRING(PONO,8,6) AS COUNTNUMBER,B.FINANCIALYEAR FROM [T_TPU_POHEADER] A WITH (NOLOCK),[P_APP_PONO_MASTER] B WHERE PONO IN(SELECT MAX(PONO) AS PONO FROM dbo.T_TPU_POHEADER WITH (NOLOCK) WHERE TPUID=" + TPUID + ")";
        ...
        {
            string sql = "select B.PONO,convert(varchar(20),B.PODATE,103) AS PODATE,CATEGORYID,CATEGORYName,DIVISIONID,DIVISIONName,PACKINGSIZEID,PACKINGSIZEName as PRODUCTPACKINGSIZE,NATUREOFPRODUCTID,NATUREOFPRODUCTNAME," +
                ...
                "UOMID,UOMName,PRODUCTID,PRODUCTName,ISNULL(QTY,0) as PRODUCTQTY,ISNULL(RATE,0) as PRODUCTPRICE,ISNULL(PRODUCTCONVERSIONQTY,0) AS PRODUCTCONVERSIONQTY,ISNULL(PRODUCTAMOUNT,0) AS PRODUCTAMOUNT,CONVERT(VARCHAR(10),CAST(REQUIREDDate AS DATE),103) AS REQUIREDDate,CONVERT(VARCHAR(10),CAST(REQUIREDTODate AS DATE),103) AS REQUIREDTODate  from dbo.T_TPU_PODETAILS A WITH (NOLOCK)" +
                "INNER JOIN [T_TPU_POHEADER] B WITH (NOLOCK)  on A.POID=B.POID where B.PONO='" + pono + "' AND B.FINYEAR='" + finyear + "' ORDER BY PRODUCTName";
            DataTable dt = new DataTable();
        ...
        {
            string sql = " SELECT ISNULL(GROSSTOTAL,0) AS GROSSTOTAL,ISNULL(ADJUSTMENT,0) AS ADJUSTMENT,ISNULL(DISCOUNTPERCENTAGE,0) AS DISCOUNTPERCENTAGE,ISNULL(DISCOUNT,0) AS DISCOUNT,ISNULL(TOTALCASEPACK,0) AS TOTALCASEPACK,ORDERTYPEID," +
                         ...
                         " ISNULL(TOTALAMOUNT,0) AS TOTALAMOUNT,ISNULL(NETTOTAL,0) AS NETTOTAL,ISNULL(MRPTOTAL,0) AS MRPTOTAL,H.TPUID,CONVERT(VARCHAR(10),CAST(H.PODATE AS DATE),103) AS PODATE,H.REFERENCEPOID,H.REMARKS FROM [T_TPU_POFOOTER] F WITH (NOLOCK) " +
                         " INNER JOIN [T_TPU_POHEADER] H WITH (NOLOCK) ON F.POID=H.POID where H.PONO='" + pono + "' AND H.FINYEAR='" + finyear + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        ...
        public DataTable BindPurchaseOrderGrid()
        {
            ...
            dtPORecord.Columns.Add("PRODUCTPACKINGSIZEID");
            dtPORecord.Columns.Add("PRODUCTPACKINGSIZE");
            dtPORecord.Columns.Add("PRODUCTCONVERSIONQTY");
        ...
        public int PORecordsCheck(string PID, string PackSizeID, string PDATE, string PTODATE, string hdnvalue)
        {
            ...
            else
            {
                ...
                foreach (DataRow dr in dteditporecord.Rows)
                {
                    ...
                    {
                        if (dr["PRODUCTID"].ToString() == PID && dr["PACKINGSIZEID"].ToString() == PackSizeID)
                        {
                            flag = 0;
                            break;
                        }
        ...
        public int PORecordsDelete(string PID, string pono, string fromDate, string ToDate)
        {
            ...
            if (pono == "")
            {
                ...
                while (i >= 0)
                {
                    ...
                    {
                        grossvalue = Convert.ToDecimal(dtPORecord.Rows[i]["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dtPORecord.Rows[i]["PRODUCTPRICE"]);
                        MRPtotalDelete = Convert.ToDecimal(dtPORecord.Rows[i]["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dtPORecord.Rows[i]["MRP"]);
                        ExcisetotalDelete = (MRPtotalDelete * (Convert.ToDecimal(dtPORecord.Rows[i]["ASSESMENTPERCENTAGE"]) / 100) * (Convert.ToDecimal(dtPORecord.Rows[i]["EXCISE"]) / 100));
                        csttotalDelete = ((grossvalue + ExcisetotalDelete) * (Convert.ToDecimal(dtPORecord.Rows[i]["CST"]) / 100));
                        //totalDelete = grossvalue + ExcisetotalDelete + csttotalDelete;
            ...
            else
            {
                ...
                while (i >= 0)
                {
                    ...
                    {
                        //decimal grossvalue = Convert.ToDecimal(dteditporecord.Rows[i]["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dteditporecord.Rows[i]["PRODUCTPRICE"]);
                        grossvalue = Convert.ToDecimal(dteditporecord.Rows[i]["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dteditporecord.Rows[i]["PRODUCTPRICE"]);
                        MRPtotalDelete = (Convert.ToDecimal(dteditporecord.Rows[i]["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dteditporecord.Rows[i]["MRP"]));
        ...
        public int POEditedRecordDELETE_KKG(string pono, string finyear)
        {
            int delflag = 0;

            string sqleditedprocdetail = "exec [SP_MM_TOTALPODELETE_kkg] '" + pono + "','" + finyear + "'";
        ...
        public DataTable BindPOEditedGridRecords(string pono, string PID, string PNAME, decimal PQTY, string CONVERTIONQTY, string PPACKINGSIZE, string PPACKINGSIZENAME, string PDATE, string PTODATE, decimal PPRICE, decimal MRP, decimal ASSESMENT, decimal EXCISE, decimal CST)
        {
        ...
        public DataTable BindPurchaseOrderGridRecords(string PID, string PNAME, decimal PQTY, string CONVERTIONQTY, string PPACKINGSIZE, string PPACKINGSIZENAME, string PDATE, string PTODATE, decimal PPRICE, decimal MRP, decimal ASSESMENT, decimal EXCISE, decimal CST)
        {
            dtPORecord = (DataTable)HttpContext.Current.Session["PORECORDS"];
            ...
            dr["PRODUCTQTY"] = PQTY;
            dr["PRODUCTPACKINGSIZEID"] = PPACKINGSIZE;
            dr["PRODUCTPACKINGSIZE"] = PPACKINGSIZENAME;
            dr["PRODUCTCONVERSIONQTY"] = Convert.ToDecimal(CONVERTIONQTY);
            dr["PRODUCTAMOUNT"] = (PPRICE * Convert.ToDecimal(CONVERTIONQTY)).ToString("0.00");
            dr["PRODUCTPRICE"] = PPRICE.ToString("0.00");
        ...
        public DataTable BindPOValues(string hdnvalue)
        {
            ...
            if (hdnvalue == "")
            {
                ...
                foreach (DataRow dr in dtPORecord.Rows)
                {
                    grosstotal = grosstotal + (Convert.ToDecimal(dr["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dr["PRODUCTPRICE"]));
                    MRPValue = (Convert.ToDecimal(dr["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dr["MRP"]));
                    MRPtotal = MRPtotal + (Convert.ToDecimal(dr["PRODUCTCONVERSIONQTY"]) * Convert.ToDecimal(dr["MRP"]));
        ...
        public DataTable InsertPODetails(string podate, string tpuid, string tpuname, string remarks, string createdby, decimal grosstotal, decimal adjustment, decimal dispercent, decimal discount, decimal packpercent, decimal packing, decimal expercent, decimal exercise, decimal salepercent, decimal saletax, decimal nettotal, decimal othercharges, decimal totalamount, string hdnvalue, string finyear, decimal MRPTOTAL, string ReferencePOID, string ISAmendment, decimal CasePack, string OrderTypeID, string OrderTypeName)
        {
            int detailflag = 0;
            string flag = "";
            ...
            if (hdnvalue == "")
            {
                ...
                if (dtpono.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtPORecord.Rows.Count; i++)
                    {
                        ...
                        string productpackingsizeid = dtPORecord.Rows[i]["PRODUCTPACKINGSIZEID"].ToString();
                        string productpackingsizename = dtPORecord.Rows[i]["PRODUCTPACKINGSIZE"].ToString();
                        decimal productqty = Convert.ToDecimal(dtPORecord.Rows[i]["PRODUCTQTY"].ToString());
                        decimal productconversionqty = Convert.ToDecimal(Convert.ToDecimal(dtPORecord.Rows[i]["PRODUCTCONVERSIONQTY"].ToString()));
            ...
            else
            {
                ...
                if (dtpono.Rows.Count > 0)
                {
                    string sql = "DELETE FROM dbo.T_TPU_PODETAILS where POID IN(SELECT POID FROM T_TPU_POHEADER WHERE PONO='" + hdnvalue + "' AND FINYEAR='" + finyear + "')";
                    ...
                    for (int i = 0; i < dteditporecord.Rows.Count; i++)
                    {
                        ...
                        decimal productconversionqty = Convert.ToDecimal(Convert.ToDecimal(dteditporecord.Rows[i]["PRODUCTCONVERSIONQTY"].ToString()));
        ...
        public string FetchPoStatus(string POID)
        {
            string Sql = "SELECT ISVERIFIED FROM T_MM_POHEADER WITH(NOLOCK) WHERE POID='" + POID + "'";
            return ((string)db.GetSingleValue(Sql));
        }
        ...
        public DataTable BindReferencePO(string POID, string FinYear)
        {
            string sql = string.Empty;
            if (POID == "")
            {
                sql = "select PONO,POID from [T_TPU_POHEADER] where ISCLOSED='N' AND FINYEAR='" + FinYear + "' ORDER BY PONO DESC";
            }
            else
            {
                sql = "select PONO,POID from [T_TPU_POHEADER] where ISCLOSED='Y' AND FINYEAR='" + FinYear + "' ORDER BY PONO DESC";
            }
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        ...
        {
            string sql = "select TPUID from [T_TPU_POHEADER] where POID='" + POID + "'";
            string TPU = string.Empty;
            TPU = (string)db.GetSingleValue(sql);
            return TPU;
        }

        public DataTable FetchPODetails_Amendment(string POID)
        {
            string sql = "EXEC SP_TPU_POAMENDMENT '" + POID + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        ...
        //-------------------------///
        public DataTable FetchProductdetails(string tpuid, string PacksizeID, string PoDate)
        {
            string sql = "EXEC USP_PURCHASEORDER_GRID_DETAILS '" + tpuid + "','" + PacksizeID + "','" + PoDate + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }

        public DataTable FetchRequirementProductdetails(string tpuid, string PacksizeID, string ReqNo, string PoDate)
        {
            string sql = "EXEC USP_REQ_PO_GRID_DETAILS '" + tpuid + "','" + PacksizeID + "','" + ReqNo + "','" + PoDate + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        ...
        public string InsertMMPODetails(string hdnValue, string PODate, string TPUID, string TPUName, string Remarks, int CreatedBy,
                                        decimal GrossTotal, decimal ExciseTotal, decimal TotalCST, decimal TotalAmount, string FinYear,
                                        decimal MRPTotal, decimal TotalCase, string OrderTypeID, string OrderTypeName, string xml,
                                        string Planned, string ReqID, string ReqNo, string Depotid, string XMLuploadFILE, string fileuploadtag, string delivaryFromDate, string delivaryTodate, decimal discountAmnt, decimal freightAmnt, string extrafreight, string isVerified, decimal adjustmentAmnt, string paymentTerms)
        {
            ...
            if (hdnValue == "")
            {
                ...
                try
                {
                    string sqlprocPurchaseOrder = " EXEC [USP_PO_INSERT_UPDATE] '','" + flag + "','" + PODate + "'," +
                                                    " '" + TPUID + "','" + TPUName + "','" + Remarks + "'," +
                                                    " " + CreatedBy + "," + GrossTotal + "," +
                                                    " " + ExciseTotal + "," + TotalCST + "," + TotalAmount + "," +
                                                    " '" + FinYear + "'," + MRPTotal + "," + TotalCase + "," +
                                                    " '" + OrderTypeID + "','" + OrderTypeName + "','" + xml + "'," +
                                                    " '" + Planned + "','" + ReqID + "','" + ReqNo + "','" + Depotid + "','" + XMLuploadFILE + "','" + fileuploadtag + "','" + delivaryFromDate + "','" + delivaryTodate + "'," + discountAmnt + "," + freightAmnt + ",'" + extrafreight + "','" + isVerified + "','" + adjustmentAmnt + "','" + paymentTerms + "'";

                    DataTable dtPO = db.GetData(sqlprocPurchaseOrder);

                    if (dtPO.Rows.Count > 0)
                    {
                        POID = dtPO.Rows[0]["POID"].ToString();
            ...
            else
            {
                ...
                try
                {
                    ...
                    string sqlprocPurchaseOrder = " EXEC [USP_PO_INSERT_UPDATE] '" + hdnValue + "','" + flag + "','" + PODate + "'," +
                                                    " '" + TPUID + "','" + TPUName + "','" + Remarks + "'," +
                                                    " " + CreatedBy + "," + GrossTotal + "," +
                                                    " " + ExciseTotal + "," + TotalCST + "," + TotalAmount + "," +
                                                    " '" + FinYear + "'," + MRPTotal + "," + TotalCase + "," +
        ...
        public DataSet BindPurchaseOrderDetails(string POID)
        {
            string sql = string.Empty;
            sql = "EXEC USP_PURCHASE_ORDER_DETAILS '" + POID + "'";
            DataSet dt = new DataSet();
            dt = db.GetDataInDataSet(sql);
            return dt;
        }

        public DataSet BindPurchaseOrderDetailskkgview(string POID)
        {
            string sql = string.Empty;
            sql = "EXEC [USP_PO_ORDER_DETAILS_KKG] '" + POID + "'";
            DataSet dt = new DataSet();
            dt = db.GetDataInDataSet(sql);
            return dt;
        }
        public DataSet BindPurchaseOrderDetailskkg(string POID)
        {
            string sql = string.Empty;
            sql = "EXEC USP_PO_ORDER_DETAILS_KKG '" + POID + "'";
            DataSet dt = new DataSet();
        ...
        public DataTable FetchProductdetailsKKG(string tpuid)
        {
            string sql = "EXEC [USP_BIND_VENDOR_WISE_PRODUCT_KKG] '" + tpuid + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        public DataTable GetProductWiseRate(string poid)/*new add for duplicate po*/
        {
            string sql = "EXEC [USP_FETCH_PRODUCTDETAILS_BY_PO] '" + poid + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }

        public DataTable FetchProductdetailsKKGNEW(string tpuid)
        {
            string sql = "EXEC [USP_BIND_PRODUCT_EXCEPT_FG_KKG]'" + tpuid + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }

        public DataTable FetchSearchProductdetails(string tpuid, string SearchText)
        {
            string sql = "EXEC [USP_BIND_PRODUCT_NAME_EXCEPT_FG_KKG] '" + tpuid + "','" + SearchText + "'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
        public DataTable bindTypeWithOutFg()
        {
            string sql = "SELECT ID ,ITEM_Name FROM M_SUPLIEDITEM WITH(NOLOCK) WHERE ID <> '1'";
            DataTable dt = new DataTable();
            dt = db.GetData(sql);
            return dt;
        }
